...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: igr, nairr, nafer, nsw, nrel, icf, mgt3i, mgt_opprev, mgt10i
...
!!    read general management parameters
      read (mgtfile_num,5000) titldum
      read (mgtfile_num,*) nmgt(ihru)
      read (mgtfile_num,5000) titldum
      read (mgtfile_num,*) igro(ihru)
      read (mgtfile_num,*) ncrp
      idplt(ihru) = ncrp
      read (mgtfile_num,*) laiday(ihru)
      read (mgtfile_num,*) bio_ms(ihru)
      read (mgtfile_num,*) phu_plt(ihru)
      read (mgtfile_num,5000) titldum
      read (mgtfile_num,*) biomix(ihru)
      read (mgtfile_num,*) cn2(ihru)
      read (mgtfile_num,*) usle_p(ihru)
      read (mgtfile_num,*) bio_min(ihru)
      read (mgtfile_num,*) filterw(ihru)
      read (mgtfile_num,5000) titldum
      read (mgtfile_num,*) iurban(ihru)
      read (mgtfile_num,*) urblu(ihru)
      read (mgtfile_num,5000) titldum
      read (mgtfile_num,*) irrsc(ihru)
      read (mgtfile_num,*) irrno(ihru)
      read (mgtfile_num,*) flowmin(ihru)
      read (mgtfile_num,*) divmax(ihru)
      read (mgtfile_num,*) flowfr(ihru)
      read (mgtfile_num,5000) titldum
      read (mgtfile_num,*) ddrain(ihru)
      read (mgtfile_num,*) tdrain(ihru)
      read (mgtfile_num,*) gdrain(ihru)
      read (mgtfile_num,5000) titldum
 !!     read (mgtfile_num,*) nrot(ihru)
      read (mgtfile_num,5000) titldum 
      read (mgtfile_num,5000) titldum
...
!        do irot = 1, nrot(ihru)
          imp_trig(ihru) = 0        !|release/impound action code:
!!                                    |0 begin impounding water
!!                                    |1 release impounded water
!        end do
!     end if
...
!!    set values for cover/crop already growing
      idplt(ihru) = ncrp                !|land cover identification number
      if (igro(ihru) == 1) then
        !igrow = 1
        idplt(ihru) = ncrp
        phuacc(ihru) = .1               !why set phuacc at 0.1 for grown crops?
	    icr(ihru) = 1                     !|sequence number of crop grown within the current year
        ...
        if (hvsti(ncrp) < 1.) then
          tnylda(ihru) = 350. * cnyld(ncrp) * bio_e(ncrp)               !|kg N/kg yield |estimated/target nitrogen content of
!!                                                                      |yield used in autofertilization
        else
...
!!    areas. This assumes CN2 given in mgt file is for pervious area only
      if (iurban(ihru) > 0) then
        disc = 0.
        disc = fimp(urblu(ihru)) - fcimp(urblu(ihru))               !!    fcimp(:)   |fraction         |fraction of HRU area that is classified
                                                                    !!                                 |as directly connected impervious
                                                                    !!    fimp(:)    |fraction         |fraction of HRU area that is
                                                                    !!                                 |impervious (both directly and
                                                                    !!                                 |indirectly connected)
...
!!    Filter strip calculations
      if (filterw(ihru) > 0.) then                                  !|m             |filter strip width for bacteria transport
        fsred(ihru) = 1. - ((12. + 4.5 * filterw(ihru)) / 100.)     !|reduction in bacteria loading from filter strip
...
!!      read scheduled management practices
        do                                      !! operation loop
          ...
          mgt8 = 0.
          mgt9 = 0.
          read (mgtfile_num,5200,iostat=eof) mon, day, husc, mgt_op, mgt1i, mgt2i, mgt3i, mgt4, mgt5, mgt6, mgt7, mgt8, mgt9, mgt10i
          if (eof < 0) then     !If the value of IOstatus is zero, the previous READ was executed flawlessly and all variables have received their input values. This is the normal case.
                                !If the value of IOstatus is positive, the previous READ has encountered some problem.
                                !If the value of IOstatus is negative, it means the end of the input has reached.
          ...
          !! initial biomass carbon and nutrients  qichun   forestry--------------------------------------      
           if(ifor==1) then     
          if ( idplt(ihru) == 6 ) then
              bio_ms(ihru) = 100000.
               if (mgt_op > 2) mgt_op = 0
              plantn(ihru) = pltnfr(3, idplt(ihru))*bio_ms(ihru) 
              plantp(ihru) = pltpfr(3,idplt(ihru))*bio_ms(ihru) 
           end if
          
           if ( idplt(ihru) == 7 ) then
              bio_ms(ihru) = 100000.
               if (mgt_op > 2) mgt_op = 0
              plantn(ihru) = pltnfr(3, idplt(ihru))*bio_ms(ihru) 
              plantp(ihru) = pltpfr(3,idplt(ihru))*bio_ms(ihru) 
 ...
 !! above added 
        ...
        if (mgt_op == 0 .or. mgt_op == 17) then                             !! mgt_op if
              ...
              if (mgt_op == 1) then
	            ...
	            if (mgt_op == 4 .or. mgt_op == 15) then
	              ...
	              hrupest(ihru) = 1
                    ...
                    if (newpest == 0) then
                       npno(npmx) = mgt1i           !|array of unique pesticides used in watershed
                       nope(mgt1i) = npmx           !|sequence number of pesticide in NPNO(:)
                     npmx = npmx + 1                !|number of different pesticides used in
                    endif
	            endif
	    end if     
	                                           !! mgt_op if        
