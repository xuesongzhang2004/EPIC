...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: xirr,solpyldjga
      integer :: j, nnro, nicr, k, sumpady, ncrp
      character*4 cropname
      if (iscen == 1) then
          write (output_std_num,6300) iyr, wshdaao(1), wshdaao(3), wshdaao(4),      &
                 wshdaao(104), wshdaao(5), wshdaao(109), wshddayo(35), &
                 wshdaao(7), wshdaao(108), wshdaao(6), wshdaao(12),   & 
                 wshdaao(42), wshdaao(45), wshdaao(46), wshdaao(44),   &
                 wshdaao(40), wshdaao(43), wshdaao(41), wshdaao(111)
    
      else if (isproj == 1) then
          write (output2_std_num,6300) iyr, wshdaao(1), wshdaao(3), wshdaao(4),      &
                 wshdaao(104), wshdaao(5), wshdaao(109), wshddayo(35), &
                 wshdaao(7), wshdaao(108), wshdaao(6), wshdaao(12),    &
                 wshdaao(42), wshdaao(45), wshdaao(46), wshdaao(44),   &
                 wshdaao(40), wshdaao(43), wshdaao(41), wshdaao(111)
      ...
      write (output_std_num,1200)
      else if (isproj == 1) then
      write (output2_std_num,1000) prog
      write (output2_std_num,1100) title
      write (output2_std_num,1200)
      endif
        xirr = 0.
        xirr = Sum(aairr)
        ...
        if (xirr > 0.) then
          if (iscen == 1) then
          write (output_std_num,1300)
          else if (isproj == 1) then
          write (output2_std_num,1300)
          endif
          do j = 1, nhru
            if (iscen == 1) then
                if (aairr(j) > 1.e-4) write (output_std_num,1400) j, irn(j), aairr(j)
            else if (isproj == 1) then
                if (aairr(j) > 1.e-4) write (output2_std_num,1400) j, irn(j), aairr(j)
            endif
          end do
        end if
...
!      end if
      if (iscen == 1) then
          write (output_std_num,1000) prog
          write (output_std_num,1100) title
          write (output_std_num,1500)
      else if (isproj == 1) then
          write (output2_std_num,1000) prog
          write (output2_std_num,1100) title
          write (output2_std_num,1500)
      endif
      do j = 1, nhru
        if (iscen == 1) then
          if (mcrhru(j) > 0) then
          write (output_std_num,1600) j, hru_sub(j), (cpnm(idplrot(nicr,j)), yldn(nicr,j), bio_aahv(nicr,j),nicr = 1, mcrhru(j))
          else
          write (output_std_num,1601) j, ' BARE'
          end if
          else if (isproj == 1) then
          if (mcrhru(j) > 0) then
          write (output2_std_num,1602) j, (cpnm(idplrot(nicr,j)), yldn(nicr,j), bio_aahv(nicr,j),nicr = 1, mcrhru(j))
          else
          write (output_std_num,1601) j, ' BARE'
          end if
        endif
...
!! write average annual HRU data
      ...
      if (iscen == 1) then
          write (output_std_num,1700)
          write (output_std_num,1800)
      do j = 1, nhru
      if (idplt(j) > 0) then
        ...
        write (output_std_num,1900) j, hru_sub(j),                                 & 
      ...
      write (output2_std_num,1700)
      write (output2_std_num,1800)
      do j = 1, nhru
      if (idplt(j) > 0) then
        cropname = cpnm(idplt(j))
      else
        ...
        write (output2_std_num,1900) j, hru_sub(j), cropname,                                &
          snam(j), hru_km(j), cn2(j), sol_sumfc(j), usle_ls(j),             &
          hruaao(22,j), hruaao(28,j), hruaao(29,j), sumix(j),          &
          hruaao(1,j), hruaao(19,j), hruaao(5,j) + hruaao(6,j),        &
          hruaao(12,j), hruaao(14,j), hruaao(37,j) + hruaao(38,j),     &
...
!! write average annual watershed monthly values
      if (iscen == 1) then
      write (output_std_num,2000)
      do j = 1, 12
        write (output_std_num,2100) j, (wshd_aamon(j,k), k = 1, 8)
      end do
      else if (isproj == 1) then
      write (output2_std_num,2000)
      do j = 1, 12
        write (output2_std_num,2100) j, (wshd_aamon(j,k), k = 1, 8)
      end do
      endif
	
!! write average annual stress values
      if (iscen == 1) then
      write (output_std_num,2200) wshd_wstrs, wshd_tstrs, wshd_nstrs, wshd_pstrs, wshd_astrs 
      else if (isproj == 1) then
      write (output2_std_num,2200) wshd_wstrs, wshd_tstrs, wshd_nstrs, wshd_pstrs, wshd_astrs 
      endif
...
!! watershed summary water balance table
      ...
      write (output_std_num,1000) prog
      write (output_std_num,1100) title
      write (output_std_num,2300) wshdaao(1), wshdaao(39), wshdaao(36), wshdaao(37),    &
 ...
 !     write (output_std_num,2400) wshdaao(33), wshdaao(15), wshdaao(34), wshdaao(11)
      else if (isproj == 1) then
      write (output2_std_num,1000) prog
      write (output2_std_num,1100) title
      write (output2_std_num,2300) wshdaao(1), wshdaao(39), wshdaao(36), wshdaao(37),    &
         wshdaao(3), wshdaao(4),wshdaao(109),wshdaao(104),wshdaao(113),     &
         wshdaao(105), wshdaao(106), wshdaao(107), wshdaao(6),              &    
         wshdaao(5), wshdaao(7), wshdaao(108), wshdaao(38),                 &
         wshdaao(12)
 ...
 !    &    wshdaao(23), wshdaao(14), (resdata(k),k = 1,7)
 !     write (output2_std_num,2400) wshdaao(33), wshdaao(15), wshdaao(34), wshdaao(11)
      endif
...
!      if (sumpady > 0) then
        if (iscen == 1) then
        write (output_std_num,2500) spadyo, spadyev, spadysp, spadyosp
        else if (isproj == 1) then
        write (output2_std_num,2500) spadyo, spadyev, spadysp, spadyosp
        endif
!     end if
...
!! watershed summary nutrient table
      ...
      write (output_std_num,1000) prog
      write (output_std_num,1100) title
      write (output_std_num,2600)
      write (output_std_num,2700)wshdaao(40), wshdaao(41), wshdaao(42), wshdaao(45),        &
         wshdaao(111), wshd_ptile, wshd_pinlet,                             &
         wshdaao(43), wshdaao(46), wshd_plch, wshdaao(44), wshd_pup,        &
         wshdaao(110)
      write (output_std_num,2800) wshd_pal, wshd_pas, wshd_ftotn, wshd_ftotp,  &
      		wshd_fixn, wshd_dnit
      write (output_std_num,2900) wshd_hmn, wshd_rwn, wshd_hmp, wshd_rmn, wshd_rmp
      write (output_std_num,3000) wshd_raino3, basno3i, basno3f, basorgni, basorgnf,            &
      ...
      write (output2_std_num,1000) prog
      write (output2_std_num,1100) title
      write (output2_std_num,2600)
      write (output2_std_num,2700) wshdaao(40), wshdaao(41), wshdaao(42),            &
         wshdaao(45), wshdaao(43), solpyldjga, tile_solpo(j),           &
         wshdaao(46), wshd_plch, wshdaao(44),                           &
         wshd_pup, wshdaao(110)
      write (output2_std_num,2800) wshd_pal, wshd_pas, wshd_ftotn, wshd_ftotp,       &
         wshd_fixn, wshd_dnit
      write (output2_std_num,2900) wshd_hmn, wshd_rwn, wshd_hmp, wshd_rmn, wshd_rmp
      write (output2_std_num,3000) wshd_raino3, basno3i, basno3f, basorgni, basorgnf,&
         basminpi, basminpf, basorgpi, basorgpf, wshd_fno3,            &
         wshd_fnh3, wshd_forgn, wshd_fminp, wshd_forgp, wshd_yldn,     &
         wshd_yldp, wshd_voln, wshd_nitn, sno3up
      endif
...
!! watershed bacteria summary table
      if (iscen == 1) then
      write (output_std_num,3100) sdiegropq,sdiegrolpq,sdiegrops,sdiegrolps,        &
          sbactrop,sbactrolp,sbactsedp,sbactsedlp,sbactlchp,sbactlchlp
      else if (isproj == 1) then
      write (output2_std_num,3100) sdiegropq,sdiegrolpq,sdiegrops,sdiegrolps,        &
          sbactrop,sbactrolp,sbactsedp,sbactsedlp,sbactlchp,sbactlchlp
      endif

!! septic variables to output.std
      write (output_std_num,3101) wshd_sepno3, wshd_sepnh3, wshd_seporgn,           &
      wshd_sepfon, wshd_seporgp, wshd_sepfop, wshd_sepsolp,             &
      wshd_sepbod
     
       !! 
      write (output_std_num,3200) wshdaao(57), wshdaao(52), wshdaao(69)+ wshdaao(70),    &
        wshdaao(71)+wshdaao(72), wshdaao(73), wshdaao(74),                  &
       wshdaao(94)+ wshdaao(95), wshdaao(96)+ wshdaao(97),wshdaao(98),      &
       wshdaao(82)+ wshdaao(83), wshdaao(84)+ wshdaao(85),wshdaao(86),      &
       wshdaao(88)+ wshdaao(89),  wshdaao(91)+ wshdaao(92),                 &
       (wshdaao(88)+ wshdaao(89))- (wshdaao(91)+ wshdaao(92)),              &
       wshdaao(90),                                                         &
      wshdaao(75), wshdaao(76), wshdaao(99), wshdaao(100),wshdaao(101),     &
       wshdaao(99)+wshdaao(100)+wshdaao(101),                               &
      wshdaao(93),                                                          &
      wshdaao(102), wshdaao(103), wshdaao(105), wshdaao(106),               &
      wshdaao(105)- wshdaao(106) 
...
!     &    t20,'WATER = ',f8.3,' MM'/t20,'SEDIMENT = ',f7.3,' T/HA')
 ...
 6300 format (/i5,15f8.2,1x,4f8.2,60f8.2//)     !!-----------------------------------------output.std------------------------------------------------------  
   
   
 3200 format (/,t20,'NPP C=  ',f8.2,' (KG/HA)',/,t20,   &
         'SOL RESP C =  ',f8.2,' (KG/HA)',/,t20,        &
         'SUB POC =  ',f8.2,' (KG/HA)',/,t20,           &          
         'SUB DOC = ',f8.2,' (KG/HA)',/,t20,            &         
         'SUB DIC = ',f8.2,' (KG/HA)',/,t20,            &        
         'SUB ALG C = ',f8.2,' (KG/HA)',/,t20,          &           
         'RCH POC = ',f8.2,' (KG/HA)',/,t20,            &        
         'RCH DOC = ',f8.2,' (KG/HA)',/,t20,            &       
         'RCH DIC = ',f8.2,' (KG/HA)',/,t20,            &      
         'OUT POC = ',f8.2,' (KG/HA)',/,t20,            &
         'OUT DOC = ',f8.2,' (KG/HA)',/,t20,            &       
         'OUT DIC = ',f8.2,' (KG/HA)',/,t20,            &      
         'DEP POC = ',f8.2,' (KG/HA)',/,t20,            &
         'RESUS POC = ',f8.2,' (KG/HA)',/,t20,          &
         'NET DEP POC = ',f8.2,' (KG/HA)',/,t20,        &
         'DEP ALG C = ',f8.2,' (KG/HA)',/,t20,          &        
         'BURY C = ',f8.2,' (KG/HA)',/,t20,             &     
         'OUTGAS C = ',f8.2,' (KG/HA)',/,t20,           &
         'SED CO2 = ',f8.2,' (KG/HA)',/,t20,            &       
         'SED SOL CH4 = ',f8.2,' (KG/HA)',/,t20,        &          
         'SED GAS CH4 = ',f8.2,' (KG/HA)',/,t20,        &      
         'SED MIN C = ',f8.2,' (KG/HA)',/,t20,          &
         'RESUS CH4 = ',f8.2,' (KG/HA)',/,t20,          &
         'SUB SED = ',f8.2,' (TON/HA)',/,t20,           &       
         'OUT SED = ',f8.2,' (TON/HA)',/,t20,           &       
         'DEP SED  = ',f8.2,' (TON/HA)',/,t20,          &    
         'DEG SED = ',f8.2,' (TON/HA)',/,t20,           &
         'NET DEP SED = ',f8.2,' (TON/HA)',/)
