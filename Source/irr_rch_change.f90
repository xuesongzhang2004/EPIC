...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: jrch, k, flag, ii,j
      real :: cnv, vmm, vminmm, vol, wtrin, wtr_avail, vmxi, xx
      real :: min
      jrch = 0
      jrch = inum1
          ...
          if (flag > 0) then
            ...
            if (rtwtr > flowmin(k) * 86400.) then
              ...
              if (vmm > 0.) then
                vol = 0.
                vol = vmm * cnv 
                call irrigate(k,vmm)        !!R666b 7/19/17 nbs
...
!!                xx = vol     							                           !! BN: replaced "wtrin" with "vol"
            ...
            if (imgt == 1) then
             write (output_mgt_num, 1000) subnum(k), hruno(k), iyr, i_mo, iida,        &
            hru_km(k), "         ",  " AUTOIRR", phubase(k), phuacc(k),     &       !!R666b 7/19/17 nbs
