...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j,sb,kk,i_wtrhru,ihout1,iru_sub 
      real :: tmpk, d, gma, ho, pet_alpha, aphu, phuop, xx,sumdaru 
      integer :: ly 
      real:: sumk,ovsl,ovs
      ...
      if (cswat == 2 ) then
          ...
          !! DSSAT tillage
              do ly = 1, sol_nly(j)
                  tillage_days(ly,ihru) = tillage_days(ly,ihru) + 1
              end do
          end if
      ...
      if (icr(j) <= 0) icr(j) = 1       !|sequence number of crop grown within the current year
      
      i_wtrhru = 0                      !is water HRU or not.
	...
	if (i_wtrhru == 1) then
        ...
        if (tmpav(j) > 0. .and. phutot(hru_sub(j)) > 0.01) then     !|heat units    |base zero total heat units (used when no land cover is growing)
           phubase(j) = phubase(j) + tmpav(j) / phutot(hru_sub(j)) !Initialized in zero0-Du.f90(443):      phubase = 0.; 
                                                                    !simulate.f90(276):              phubase(ihru) = 0.
                                                                    !simulate.f90(344):            phubase(j) = 0.
                                                                    !zero0-Du.f90(443):      phubase = 0.
        end if
        
        call schedule_ops                                           !!this subroutine schedules BMPs. Different from operatn

        !! calculate albedo for day
        call albedo

        !! calculate soil temperature for soil layers  by Junyu Qi
...
!
!     &        then             
          !! calculate surface runoff if HRU is not impounded or an undrained depression--
          call surface          !!subdaily calculation starts here. Note: allocate (precipdt(nstep+1)); precipdt(:) |mm H2O      |precipitation in time step for HRU
                                !!surface.f90(86):          precipdt(ii+1) = precipdt(ii+1) + ovrlnd_dt(j,ii)
                                !!varinit.f90(280):             precipdt(ii+1) = rainsub(j,ii)
...
!        end if
        ...
        !call rain_pH        !! added by Junyu Qi
         
         
        !! perform management operations
        if (yr_skip(j) == 0) call operatn !contains "sched_mgt"
                                         !zero0-Du.f90(551):      yr_skip = 0; sched_mgt.f90(328):            yr_skip(j) = 1; simulate.f90(350):              yr_skip(j) = 1
...
!        if (pot_vol(j) < 1.e-6) call etact
        ...
        if (icfrt(j) == 1) then 
          ndcfrt(j) = ndcfrt(j) + 1 !|days          |number of days HRU has been continuously fertilized
          call confert
        end if
...
!12112  format (2i4,12f8.2)
        ...
        if (cswat == 0) then
            call nminrl         ! call ndenit(k,j,cdg,wdn,0.05)
	    end if
	    if (cswat == 1) then
		    call carbon      ! call ndenit(k,j,cdg,wdn,0.05)
	    end if
	    if (cswat == 2) then 
	      call carbon2
	    end if
       ...
       !!Urea hydrolysis  added by Junyu Qi 04/2021
        call fern_hyd     
      ...
      !! Denitrification
       ...
       !! Nitrification 
        select case( nswat )
          case(0)  !! Original nitrification method
           call nitvol
           case(1)  !  Daycent et al.
           call nitvol_new
        end select
...
!!      if 1)current is septic hru and 2)  soil temperature is above zero
        ...
        call pestlch
         ...
         !if (surfq(j) > 0. .and. peakr > 1.e-6) then       !! Disabled by Junyu Qi
         ! if (precipday > 0.) then                                  !! Disabled by Junyu Qi
            call enrsb(0)
...
!        endif
        ...
        call substor
         ...
         if (iwtmp_sub == 0)  call hru_wtmp      
       
         if( tswat==1 .and. iwtmp_sub==2 )then
         
            call hru_surfwtmp                 ! surface water temperature   Junyu Qi
            call hru_solwtmp                  ! soil water temperatrue from HRU  Junyu Qi 
       
         end if
       
         if( iwtmp_sub==1 )  call hru_wtmp_Ficklin
      ...
      !!  latq_ru(j) = 0.
        ...
        call subwq
!-----------------------Calculating RDOC and DIC loads from HRU--------------------------------------------!


        HRU_RDOC(j) = SurQ_DOC(j)+ LatQT_DOC(j)+  GwQ_DOC(j)          !calculating total RDOC from HRU (kg/ha)
        HRU_DIC(j) = SurQ_DIC(j)+ LatQT_DIC(j)  + GwQ_DIC(j)          !calculating total DIC from HRU (kg/ha)         
        HRU_RDON(j) = SurQ_DON(j)+ LatQT_DON(j)+  GwQ_DON(j)      
        HRU_DIN(j) = surqno3(j) + latno3(j)+ no3gw(j)+ tileno3(j)                        !calculating total DIN from HRU (kg/ha)    
        HRU_DIP(j) = surqsolp(j)+minpgw(j)+sedminpa(j)+sedminps(j)                 !calculating total DIP from HRU (kg/ha)        
!-----------------------Calculating RDOC and DIC loads from HRU--------------------------------------------!
        !! compute wetland processes
        call wetlan_Du                         !! modified by Du

        !! compute pond processes
        if (ievent == 0) then
           call hrupond_Du                    !! modified by Du
        else
           call hrupondhr
        endif
