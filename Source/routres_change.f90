...
!!    ires_code = 1 turn off reservoirs for santhi
      ...
      if (ires_code == 1) then 
        ...
        if (cswat == 2) then
         varoute(33,ihout) = varoute(33,inum2)  !!RPOC
         varoute(34,ihout) = varoute(34,inum2)  !!LPOC
         varoute(35,ihout) = varoute(35,inum2)  !!RDOC
         varoute(36,ihout) = varoute(36,inum2)  !!LDOC
         varoute(37,ihout) = varoute(37,inum2)  !!DIC
        endif
      !---------------For carbon module,added by Du-----------------------!
      ...
      if (iyr > iyres(jres) .or.      &                                  
        ...
        If(ievent_rch == 0) then		!! urban modeling by J.Jeong
	        call res
	    else 
	        call reshr
	    end if
  ...
  !!      call res
        ...
        call resnut_Du               !! Daily reservori nutirent cycles modified by Du
        ...
        varoute(21,ihout) = varoute(21,inum2)  !!conservative metal #2
        varoute(22,ihout) = varoute(22,inum2)  !!conservative metal #3
      !--------Calculating carbon amount leaving the reservoir by outflow,added by Du--------!
        if (cswat == 2) then
           varoute(33,ihout) = resrpoc  !! RPOC leaving the reservoir on day (kg C)
           varoute(34,ihout) = reslpoc  !! LPOC leaving the reservoir on day (kg C)
           varoute(35,ihout) = resrdoc  !! RDOC leaving the reservoir on day (kg C)
           varoute(36,ihout) = resldoc  !! LDOC leaving the reservoir on day (kg C)
           varoute(37,ihout) = resdic   !! DIC leaving the reservoir on day (kg C) 
        endif
      !--------Calculating carbon amount leaving the reservoir by outflow,added by Du--------!
        if (ievent_rch > 0) then
          do ii = 1, nstep_rch
            ...
            hhvaroute(4,ihout,ii) = resorgno / real(nstep_rch)
            hhvaroute(5,ihout,ii) = resorgpo / real(nstep_rch)
            hhvaroute(6,ihout,ii) = resno3o / real(nstep_rch)
            hhvaroute(7,ihout,ii) = ressolpo / real(nstep_rch)
            hhvaroute(8,ihout,ii) = 0.           !!undefined
            hhvaroute(9,ihout,ii) = 0.           !!undefined
            hhvaroute(10,ihout,ii) = 0.          !!undefined
            hhvaroute(11,ihout,ii) = solpesto / real(nstep_rch)
            hhvaroute(12,ihout,ii) = sorpesto / real(nstep_rch)
            hhvaroute(13,ihout,ii) = reschlao / real(nstep_rch)
            hhvaroute(14,ihout,ii) = resnh3o / real(nstep_rch)
            hhvaroute(15,ihout,ii) = resno2o / real(nstep_rch)
            ...
            hhvaroute(20,ihout,ii) = varoute(20,inum2) / real(nstep_rch) !!cons metal #1
            hhvaroute(21,ihout,ii) = varoute(21,inum2) / real(nstep_rch) !!cons metal #2
            hhvaroute(22,ihout,ii) = varoute(22,inum2) / real(nstep_rch) !!cons metal #3

            hhvaroute(23,ihout,ii) = varoute(23,inum2) / real(nstep_rch) !!Sand out
            hhvaroute(24,ihout,ii) = varoute(24,inum2) / real(nstep_rch) !!Silt out
            hhvaroute(25,ihout,ii) = varoute(25,inum2) / real(nstep_rch) !!clay out
            hhvaroute(26,ihout,ii) = varoute(26,inum2) / real(nstep_rch) !!Small agg out
            hhvaroute(27,ihout,ii) = varoute(27,inum2) / real(nstep_rch) !!Large agg out
            hhvaroute(28,ihout,ii) = varoute(28,inum2) / real(nstep_rch) !!Gravel out
      ...
      if (iprint == 1 .and. curyr > nyskip) then
        ...
        write (output_rsv_num,5000) jres, iida, res_vol(jres), resflwi / 86400.,  &   
        ...
        write (output2_rsv_num,5000) jres, iida, res_vol(jres), resflwi / 86400.,    &
         (resflwo / 86400.), respcp, resev, ressep, ressedi, ressedo,  &
         sedcon, varoute(4,inum2), resorgno, resorgnc,                 &
         varoute(5,inum2), resorgpo, resorgpc, varoute(6,inum2),       &
         resno3o, resno3c, varoute(15,inum2), resno2o, resno2c,        &
        ...
        write (output_rsv_num,6000) jres, iida, res_vol(jres), resflwi / 86400.,    &
         (resflwo / 86400.), respcp, resev, ressep, ressedi, ressedo,  &
         sedcon, varoute(4,inum2), resorgno, resorgnc,                 &
         varoute(5,inum2), resorgpo, resorgpc, varoute(6,inum2),       &
         resno3o, resno3c, varoute(15,inum2), resno2o, resno2c,        &
