...
!!      real :: wur, hiad1, yield, yieldn, yieldp, yldpst
      ...
      real :: wur, hiad1, resnew, yldpst,xx
...
!! calculate nutrients removed with yield
      ...
      yieldn(j) = Min(yieldn(j), 0.85 * plantn(j))
      yieldp(j) = Min(yieldp(j), 0.85 * plantp(j))
      plantn(j) = plantn(j) - yieldn(j)
      plantn(j) = amax1(0.,plantn(j))
      plantp(j) = plantp(j) - yieldp(j)
...
!! summary calculations
      ...
      if (curyr > nyskip) then
       ...
       bio_hv(icr(j),j) = yield(j) + bio_hv(icr(j),j)
