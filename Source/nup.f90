...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      un2 = 0.
      un2 = pltfr_n(j) * bio_ms(j)              !|kg N/ha       |ideal plant nitrogen content
      if (un2 < plantn(j)) un2 = plantn(j)
      uno3d = un2 - plantn(j)
      uno3d = Min(4. * pltnfr(3,icrop) * bioday, uno3d) !|kg N/ha       |plant nitrogen deficiency for day in HRU
      ...
      do ly = 1, sol_nly(j)
        ...
        gx = 0.
        if (sol_rd <= sol_z(ly,j)) then  !|mm            |current rooting depth
          gx = sol_rd
          ir = 1
        else
          gx = sol_z(ly,j)
        ...
        unmx = 0.       !!    unmx        |kg N/ha       |maximum amount of nitrogen that can be removed from soil layer
        uno3l = 0.      !!    uno3l       |kg N/ha       |amount of nitrogen removed from soil layer
        unmx = uno3d * (1. - Exp(-n_updis * gx / sol_rd)) / uobn           !none           |nitrogen uptake distribution parameter 
                                                                            !!    uobn        |none           |nitrogen uptake normalization parameter                                                                            
        uno3l = Min(unmx - nplnt(j), sol_no3(ly,j))                         !kg N/ha       |plant uptake of nitrogen in HRU for the day
        !uno3l = up_reduc * uno3l
        nplnt(j) = nplnt(j) + uno3l 
        sol_no3(ly,j) = sol_no3(ly,j) - uno3l
        
        no3_up(j) = no3_up(j) + uno3l !! Zhang added
      end do
