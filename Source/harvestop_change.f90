...
!!    real :: hiad1, wur, yield, clip, yieldn, yieldp, clipn, clipp
      ...
      real :: clipgrn,cliptbr,clipngrn,clippgrn,yieldngrn, clipngrp
      ...
      real :: RLN, RLR, orgc_f
      real :: clipbms, clipnbms, clippbms, yieldnbms, yieldpbms
      real :: rtresn, ssb, SSABG, SSR, SSN, SSP, XX, ff3
      ...
      ssb = bio_ms(j)                            !bio_ms(:)   |kg/ha          |land cover/crop biomass (dry weight)! Armen 16 Jan 2009 storing info
      ...
      tnyld(j) = (1. - rwt(j)) * bio_ms(j) * pltfr_n(j) * auto_eff(j) !auto_eff(:) |none           |fertilizer application efficiency calculated as the amount of N applied divided by the amount of N removed at harvest
 ...
 !!       yieldtbr = bio_ms(j) * (1. - 1. / (1. + hiad1))
        ...
        yieldtbr(j) = bio_ms(j) * (1. - 1. / (1. + hiad1)) * harveff  !! corrected by cibin Nov/2013   1|actual harvest index (adj for water/growth)
        cliptbr = bio_ms(j) * (1. - 1. / (1. + hiad1)) * (1. - harveff) !! corrected by cibin Nov/2013
        bio_ms(j) = bio_ms(j) - yieldtbr(j) - cliptbr
        !! calculate nutrients removed with yield
        yieldntbr = yieldtbr(j) * cnyld(idplt(j))
        yieldptbr = yieldtbr(j) * cpyld(idplt(j))
	...
	if (hi_bms > 0.) then       !! compute biomass yield !! corrected by cibin Nov/2013
        yieldbms(j) = hi_bms * (1.-rwt(j)) * bio_ms(j)*harveff
        clipbms = hi_bms * (1.-rwt(j)) * bio_ms(j) * (1. - harveff)  
        bio_ms(j) = bio_ms(j) - yieldbms(j) - clipbms !corrected by Jaehak Jeong sep. 2013
        !! calculate nutrients removed with yield
        yieldnbms = yieldbms(j) * cnyld(idplt(j))   !! corrected by cibin Nov/2013
        yieldpbms = yieldbms(j) * cpyld(idplt(j))
        ...
        yieldgrn(j) = (1.-rwt(j)) * bio_ms(j) * hiad1* harveff
        !! determine clippings (biomass left behind) and update yield

        clipgrn = (1.-rwt(j)) * bio_ms(j) * hiad1 * (1. - harveff)
        bio_ms(j) = bio_ms(j) - yieldgrn(j) - clipgrn
        !! calculate nutrients removed with yield
        yieldngrn = yieldgrn(j) * cnyld(idplt(j))
        yieldpgrn = yieldgrn(j) * cpyld(idplt(j))
      ...
      if (hi_rsd > 0.) then
        ...
        yieldprsd = hi_rsd * sol_fop(1,j)               !!R667 8/14/17 nbs
        sol_rsd(1,j) = sol_rsd(1,j) - yieldrsd(j)
        sol_fon(1,j) = sol_fon(1,j) - yieldnrsd
        sol_fop(1,j) = sol_fop(1,j) - yieldprsd
      end if

      yield(j) = yieldgrn(j) + yieldbms(j) + yieldtbr(j) + yieldrsd(j)
      yieldn(j) = yieldngrn + yieldnbms + yieldntbr + yieldnrsd
      yieldp(j) = yieldpgrn + yieldpbms + yieldptbr + yieldprsd
      ...
      if (cswat == 2) then
        grainc_d(j) = grainc_d(j)+ (yield(j)-yieldrsd(j)) * CFB  
        rsdc_d(j) = rsdc_d(j)+(clip-yieldrsd(j)) * CFB 
        rsdn_d(j) = rsdn_d(j)+clipngrn+clipnbms+clipntbr-yieldnrsd 
      end if
      !!add by zhang
      !!=====================
      ...
      OrgC_Plt2Rsd(j)=OrgC_Plt2Rsd(j)+(clipgrn+clipbms+cliptbr-yieldrsd(j))*CFB    !!residue C input
                                  
      OrgN_Plt2Rsd(j)=OrgN_Plt2Rsd(j)+clipngrn+clipnbms+clipntbr-yieldnrsd      !!residue N input 
      OrgP_Plt2Rsd(j)=OrgP_Plt2Rsd(j)+clipngrp+clippbms+clipptbr-yieldprsd      !!residue P input
      grainN(j)= yieldn(j) - yieldnrsd                                             !!    
           
           
        resnew = clip- yieldrsd(j)      !!
        resnew_n = clipn - yieldnrsd  !! 
      ...
      if (cswat == 2 .and. resnew > 0.) then
        ...
        !RLN = 1000*(resnew * CLG/(resnew_n+1.E-5)) !Not sure 1000 should be here or not!
	...
	!! Calculation for dead roots allocations, resetting phenology, updating other pools
      if (ssabg > 1.e-6) then
        ff3 = (yield(j) + clip) / ssabg	! Armen 20 May 2008 and 16 Jan 2009
      else
        ff3 = 1.
      endif 
      if (ff3 > 1.0) ff3 = 1.0
...
!     rtfr = 0.
      ...
      do ly=1, sol_nly(j)
        ...
        sol_fon(ly,j) = sol_fon(ly,j) + rtfr(ly) * rtresn
        sol_fop(ly,j) = sol_fop(ly,j) + rtfr(ly) * rtresp

        !!insert new biomss by zhang
        !!=============================
        if (cswat == 2 .and. rtresnew > 0.) then
              rsdc_d(j) = rsdc_d(j) + rtfr(ly) * rtresnew * CFB
              rsdn_d(j) = rsdn_d(j) + rtfr(ly) * rtresn
              BLG3 = 0.10
              BLG1 = 0.01/0.10
              BLG2 = 0.99
              ...
              !sol_LSLC(l,j) = sol_LSLC(l,j) + RLR * CFB * LSF * resnew
              ...
              !update no3 and nh3 in soil
              absorbed_no3(j) = absorbed_no3(j) + sol_no3(ly,j) * abs_f(j)      !!Zhang added
              absorbed_nh3(j) =absorbed_nh3(j) + sol_nh4(ly,j) * abs_f(j)       !!Zhang added            
	...
	!! summary calculations
      if (curyr > nyskip) then
       ...
       !   case default
        bio_hv(icr(j),j) = (yield(j) + clip + rtresnew) + bio_hv(icr(j),j)                       !! Jeff, is this the intention
        bio_yrms(j) = bio_yrms(j) + (yield(j) + clip + rtresnew) / 1000.		            !! Jeff, is this the intention
       ! end select
      endif
