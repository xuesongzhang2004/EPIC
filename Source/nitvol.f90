	    ...
	    !! apply septic algorithm only to active septic systems
          if(k/=i_sep(j).or.isep_opt(j)/= 1) then  ! J.Jeong for septic, biozone layer
             if (rvol + rnit > 1.e-6) then
               ...
               rnit = rnv - rvol
               if (rnit < 0.) rnit = 0.
               sol_nh4(k,j) = Max(1.e-6, sol_nh4(k,j) - rnit)
             endif
             if (sol_nh4(k,j) < 0.) then
               rnit = rnit + sol_nh4(k,j)
               sol_nh4(k,j) = 0.
             endif
             sol_no3(k,j) = sol_no3(k,j) + rnit
             
             no3_nitr(j) = no3_nitr(j) + rnit

             !! calculate ammonia volatilization
             sol_nh4(k,j) = Max(1.e-6, sol_nh4(k,j) - rvol)
             if (sol_nh4(k,j) < 0.) then
               rvol = rvol + sol_nh4(k,j)
               sol_nh4(k,j) = 0. 
             endif
             nh4_vol(j) = nh4_vol(j) + rvol
