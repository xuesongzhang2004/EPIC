...
!!    revap is subtracted and rchrg is delayed (johnson, 1977)
      ...
      rchrg1 = 0.
      rchrg_karst = 0.
      rchrg1 = rchrg(j) + rchrg_src(j)  !watuse.f90(58):        rchrg_src(j) = -1. * wushal(i_mo,j) * 10000. / cnv
                                        !wushal(:,:) |10^4 m^3/day  |average daily water removal from the shallow

!! add seepage from secondary channels, ponds, and wetlands;
       rchrg_karst = tloss + twlpnd(j) + twlwet(j)   !!R669 6/20/18 nbs   !twlpnd      |mm H2O        |water lost through seepage from ponds on on day in HRU
                                                                          !twlwet      |mm H2O        |water lost through seepage from wetlands on day in HRU    
      twlpnd(j) = 0.             !!R669 6/20/18 nbs
      twlwet(j) = 0.             !!R669 6/20/18 nbs
...
!! compute deep aquifer level for day
      gwseep = rchrg(j) * rchrg_dp(j)           !rchrg_dp(:) |none          |recharge to deep aquifer: the fraction of root zone percolation that reaches the deep aquifer
      deepst(j) = deepst(j) + gwseep            !gwseep      |mm H2O        |amount of water recharging deep aquifer on current day in HRU

      shallst(j) = shallst(j) + (rchrg(j) - gwseep)
      gwht(j) = gwht(j) * alpha_bfe(j) + rchrg(j) * (1. - alpha_bfe(j)) / (800. * gw_spyld(j) * alpha_bf(j) + 1.e-6)
      gwht(j) = Max(1.e-6, gwht(j))

!! compute groundwater contribution to streamflow for day
      if (shallst(j) > gwqmn(j)) then
        gw_q(j) = gw_q(j) * alpha_bfe(j) + (rchrg(j) - gwseep ) * (1. - alpha_bfe(j))
      else
        gw_q(j) = 0.
      end if
...
!! compute revap to soil profile/plant roots
      revapday = gw_revap(j) * pet_day          !readgw.f90(86):      read (gwfile_num,*) gw_revap(ihru)
