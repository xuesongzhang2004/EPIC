...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: idlst, j, iix, iiz, ic, mon, ii, iyr_at, mo_at, ly, iyp, idap, l
      real :: xx
      integer :: eof
      ...
      do curyr = 1, nbyr
        ...
        call sim_inityr     !sim_initday-Qi.f90(216):      mo_chk = i_mo
       ...
       if (ifirstatmo == 1) then
        ...
        do i = id1, idlst                            !! begin daily loop
          ...
          if (i_mo /= mo_chk) then          !mo_chk        |none      |current month of simulation
            immo = immo + 1                 !i_mo        |none        |month being simulated
          ...
          do j = 1, mhru
            do ly = 1, sol_nly(j)
              sol_sumno3(j) = sol_sumno3(j) + sol_no3(ly,j) + sol_nh4(ly,j)
          ...
          dtot = dtot + 1.  !is this variable used?
          nd_30 = nd_30 + 1 !the use of this variable is not clear.
          if (nd_30 > 30) nd_30 = 1

          if (curyr > nyskip) ndmo(i_mo) = ndmo(i_mo) + 1           !|ndmo days          |cumulative number of days accrued in the
                                                                    !!                               |month since the simulation began where the
                                                                    !!                               |array location number is the number of the
                                                                    !!                               |month
     
          if (pcpsim < 3) then            !|rainfall input code |1 gage read for each subbasin |2 gage simulated for each subbasin
            if (iwea==0) call clicon      !! read in/generate weather
          end if 
     
          if (iatmodep == 2) then   ! IATMODEP: 0 = average annual inputs 1 = monthly inputs  2 = daily
        ...
        do j = 1, nhru
          ...
          !! default biomix == 0.2 so computing biological mixing is default 
          !! no matter there is a tillage operation or not
          ...
          !if (idaf < 181) then
           ...
           if (iyr == mgtyr_hru(j)) then !if the mgtyr_hru is still the current calendar year, 
                                          !that means it did not encounter the end of year (mgtop == 17) yet. 
                                          !So we will execute all remaining operations (phu_op) for that year.
