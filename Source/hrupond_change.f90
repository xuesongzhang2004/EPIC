...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !! added by Du 
      real ::aa,bb,cc,dd,ee,ff,gg,hh,ii,jj
      !! added by Du
      j = 0
      j = ihru

      if (pnd_fr(j) > 1.e-6) then                   !!R671 4/5/19  nbs
        cnv = 0.
        cnv = hru_ha(j) * 10.
...
!       qdr(j) = qdr(j) - qdr(j) * pnd_fr(j)
        ...
        !--add carbon amount from HRU to pond (kg), added by Du-----------------!
        if (cswat == 2) then
            bb=Sed_LPOC(j)*xx
            cbn_pnd(j)%LPOC=cbn_pnd(j)%LPOC+Sed_LPOC(j)*xx   !(kg C)
            dd=Sed_RPOC(j)*xx
            cbn_pnd(j)%RPOC=cbn_pnd(j)%RPOC+Sed_RPOC(j)*xx   !(kg C)
            ff=HRU_RDOC(j)*xx
            cbn_pnd(j)%RDOC=cbn_pnd(j)%RDOC+HRU_RDOC(j)*xx   !(kg C)
            hh=HRU_DIC(j)*xx
            cbn_pnd(j)%DIC=cbn_pnd(j)%DIC+HRU_DIC(j)*xx      !(kg C)
            jj=HRU_LDOC(j)*xx
         endif
       !--add carbon amount from HRU to pond, added by Du---------------------! 
        ...
        !-remove carbon amoount entering ponds from HRU loadings (kg/ha),added by Du----------------!
        if (cswat == 2 ) then
...
!        qday = qday + pndflwo / cnv
        ...
        twlpnd(j) = pndsep / cnv                     !!R669 6/20/18 nbs
