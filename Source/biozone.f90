...
!!    Septic algorithm adapted from Siegrist et al., 2005
	...
	real bz_vol, rtrate,bodconc, qin, qout,qmm,qvol,pormm,rplqm
	real ntr_rt,dentr_rt, bod_rt, fcoli_rt,rtof,xx,bodi,bode
	real rnit, rdenit, rbio, rmort, rrsp, rslg, rbod, rfcoli
	real nh3_begin, nh3_end, nh3_inflw_ste, no3_begin, no3_end 
	real no3_inflow_ste, bio_steintobz,bio_outbz,bza,qi,nperc
	real hvol, solpconc, solpsorb, qlyr,qsrf
	real n2,n3,n5,n6,n7,n8,p2,p3,p4
	real solp_begin,solp_end,svolp,totalp,ctmp,plch
...
!	bio_outbz = coeff_bod_conv(j) * qout * bodconc / (1000. * bza) + (rrsp + rmort + rslg)
	...
	rnit = sol_nh4(bz_lyr,j) * (1. - Exp(-ntr_rt)) !! J.Jeong 4/03/09
	sol_nh4(bz_lyr,j) = sol_nh4(bz_lyr,j) - rnit	!J.Jeong 3/09/09
	sol_no3(bz_lyr,j) = sol_no3(bz_lyr,j) + rnit	!J.Jeong 3/09/09
	
	!ammonium percolation
	nperc = 0.2 * qout / qi * sol_nh4(bz_lyr,j)
	nperc = min(nperc,0.5*sol_nh4(bz_lyr,j))
	sol_nh4(bz_lyr,j) = sol_nh4(bz_lyr,j) - nperc
	sol_nh4(bz_lyr+1,j) = sol_nh4(bz_lyr+1,j) + nperc
  ...
  !check if max. P sorption is reached 
	...
	write(septic_out_num,1000) ihru,iyr,iida,precipday,bz_perc(j),sol_ul(bz_lyr,j),        &
