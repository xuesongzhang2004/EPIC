...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do
        read (snofile_num,1001) titldum
        if (eof < 0) exit
        read (snofile_num,1000,iostat=eof) (sub_sftmp(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (snofile_num,1000,iostat=eof) (sub_smtmp(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (snofile_num,1000,iostat=eof) (sub_smfmx(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (snofile_num,1000,iostat=eof) (sub_smfmn(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (snofile_num,1000,iostat=eof) (sub_timp(ib,i), ib = 1, 10)
        if (eof < 0) exit
        exit 
      end do
