...
!! read septic parameters
      do
        read (septfile_num,1000) titldum
	  read (septfile_num,*,iostat=eof) isep_typ(ihru)
	  if (eof < 0) exit
        if (isep_typ(ihru) <= 0) return
	  read (septfile_num,*,iostat=eof) isep_iyr(ihru)
	  if (eof < 0) exit
	  read (septfile_num,*,iostat=eof) isep_opt(ihru)       
	  if (eof < 0) exit
	  read (septfile_num,*,iostat=eof) sep_cap(ihru)
	  if (eof < 0) exit
	  read (septfile_num,*,iostat=eof) bz_area(ihru)
	  if (eof < 0) exit
	  read (septfile_num,*,iostat=eof) isep_tfail(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) bz_z(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) bz_thk(ihru)
        if (eof < 0) exit
        read (septfile_num,*,iostat=eof) sep_strm_dist(ihru)
        if (eof < 0) exit
        read (septfile_num,*,iostat=eof) sep_den(ihru)
        if (eof < 0) exit
        read (septfile_num,*,iostat=eof) bio_bd(ihru)
        if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_bod_dc(ihru)
	  if (eof < 0) exit   
        read (septfile_num,*,iostat=eof) coeff_bod_conv(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_fc1(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_fc2(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_fecal(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_plq(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_mrt(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_rsp(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_slg1(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_slg2(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_nitr(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_denitr(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_pdistrb(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_psorpmax(ihru)
	  if (eof < 0) exit
        read (septfile_num,*,iostat=eof) coeff_solpslp(ihru)
	  if (eof < 0) exit
