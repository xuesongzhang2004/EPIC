...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do k = 1, npmx
        ...
        if (iscen == 1) then
        write (output_std_num,5000) kk, pname(kk), wshd_pstap(k), wshd_pstdg(k),    &
                 wpstaao(k,1), wpstaao(k,2), wpstaao(k,3), wpstaao(k,4)
        else if (isproj == 1) then
        write (output2_std_num,5000) kk, pname(kk), wshd_pstap(k), wshd_pstdg(k),    &
                 wpstaao(k,1), wpstaao(k,2), wpstaao(k,3), wpstaao(k,4)
        endif
        ...
        pfg = pfg * 1.e6
        if (iscen == 1) then
        write (output_std_num,5100) pfp, pfg
        else if (isproj == 1) then
        write (output2_std_num,5100) pfp, pfg
        endif
      end do
