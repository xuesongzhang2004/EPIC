...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do
      read (swqfile_num,5100,iostat=eof) titldum
      if (eof < 0) exit
      read (swqfile_num,5100,iostat=eof) titldum
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rs1(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rs2(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rs3(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rs4(irch)
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rs5(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rs6(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rs7(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rk1(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rk2(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rk3(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rk4(irch)
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rk5(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) rk6(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) bc1(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) bc2(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) bc3(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) bc4(irch)
      if (eof < 0) exit
      read (swqfile_num,5100,iostat=eof) titldum
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) chpst_rea(irch)
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) chpst_vol(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) chpst_koc(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) chpst_stl(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) chpst_rsp(irch)
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) chpst_mix(irch)
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) sedpst_conc(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) sedpst_rea(irch)
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) sedpst_bry(irch) 
      if (eof < 0) exit
      read (swqfile_num,*,iostat=eof) sedpst_act(irch)
      if (eof < 0) exit
!      read (swqfile_num,*,iostat=eof) biofilm_mumax(irch)
!      if (eof < 0) exit
!      read (swqfile_num,*,iostat=eof) biofilm_kinv(irch)
!      if (eof < 0) exit
!      read (swqfile_num,*,iostat=eof) biofilm_klw(irch)
!      if (eof < 0) exit
!      read (swqfile_num,*,iostat=eof) biofilm_kla(irch)
!      if (eof < 0) exit
!      read (swqfile_num,*,iostat=eof) biofilm_cdet(irch)
!      if (eof < 0) exit
!      read (swqfile_num,*,iostat=eof) biofilm_bm(irch)
      exit
      end do
...
!! change units from day to hour if hourly routing is performed
      ...
      if (ievent_rch ==1) then
        rs1(irch) = rs1(irch) / real(nstep_rch)
        rs2(irch) = rs2(irch) / real(nstep_rch)
        rs3(irch) = rs3(irch) / real(nstep_rch)
        rs4(irch) = rs4(irch) /  real(nstep_rch)
        rs5(irch) = rs5(irch) /  real(nstep_rch)
        rk1(irch) = rk1(irch) /  real(nstep_rch)
        rk2(irch) = rk2(irch) /  real(nstep_rch)
        rk3(irch) = rk3(irch) /  real(nstep_rch)
        rk4(irch) = rk4(irch) /  real(nstep_rch)
        bc1(irch) = bc1(irch) /  real(nstep_rch)
        bc2(irch) = bc2(irch) /  real(nstep_rch)
        bc3(irch) = bc3(irch) / real(nstep_rch)
        bc4(irch) = bc4(irch) /  real(nstep_rch)
      end if
