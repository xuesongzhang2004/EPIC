...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: eof, mon, j, jj, ip,  ir, ib, ils, kk, k
      ...
      read (subfile2_num,5100) titldum
      read (subfile2_num,*) sub_km(i)
      if (isproj == 3) then                         !!R666 7/19/17 nbs
       read (subfile2_num,5101) harg_petco(i), cncoef_sub(i), sub_smfmx(1,i),        &
      ...
      read (subfile2_num,5100) titldum
      read (subfile2_num,*) sub_lat(i)
      read (subfile2_num,*) sub_elev(i)
      read (subfile2_num,*) irgage(i)
      read (subfile2_num,*) itgage(i)
      read (subfile2_num,*) isgage(i)
      read (subfile2_num,*) ihgage(i)
      read (subfile2_num,*) iwgage(i)
      read (subfile2_num,5300) wgnfile
        call caps(wgnfile)
        open (wgnfile_num,file=wgnfile)
      read (subfile2_num,*) fcst_reg(i)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (elevb(j,i), j = 1, 10)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (elevb_fr(j,i), j = 1, 10)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (ssnoeb(j), j = 1, 10)
      read (subfile2_num,*) plaps(i)
      read (subfile2_num,*) tlaps(i) 
      read (subfile2_num,*) sno_sub
      read (subfile2_num,5100) titldum
      read (subfile2_num,*) ch_ls
      read (subfile2_num,*) ch_s(1,i)
      read (subfile2_num,*) ch_w(1,i)
      read (subfile2_num,*) ch_k(1,i)
      read (subfile2_num,*) ch_n(1,i)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5300) pndfile
        call caps(pndfile)
        open (pndfile_num,file=pndfile)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5300) wusfile
        call caps(wusfile)
        open (wusfile_num,file=wusfile)
      read (subfile2_num,5100) snofile
      if(snofile /='             ' .or. snofile /= 'Climate Change')then
        if (snofile /='             ') then
          if (snofile /= 'Climate Change') then
            open (snofile_num,file=snofile)
            call caps (snofile)
            call readsno
          endif
        endif
      endif
      read (subfile2_num,*) co2(i) 
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (rfinc(i,mon),mon = 1,6)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (rfinc(i,mon),mon = 7,12)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (tmpinc(i,mon),mon = 1,6)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (tmpinc(i,mon),mon = 7,12)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (radinc(i,mon),mon = 1,6)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (radinc(i,mon),mon = 7,12)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (huminc(i,mon),mon = 1,6)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5200) (huminc(i,mon),mon = 7,12)
      read (subfile2_num,5100) titldum
!! read HRU input data
      read (subfile2_num,*) hrutot(i)
      read (subfile2_num,5100) titldum
      read (subfile2_num,5100) titldum
      read (subfile2_num,5100) titldum
      read (subfile2_num,5100) titldum
      ...
      read (subfile2_num,5100) titldum
        do j = jj, hrutot(i)
          ihru = 0
          ihru = nhru + j
          if (j == 1) hru1(i) = ihru   !hru1 is the first HRU in a subbasin
!         ipot(ihru) = ip
!         ifld(ihru) = if
!         irip(ihru) = ir
          chmfile = ""
          ...
          septfile = ""
          sdrfile = ""
         read (subfile2_num,5300) hrufile, mgtfile, solfile, chmfile, gwfile, opsfile, septfile, sdrfile, ils2(ihru)
          call caps(hrufile)
          call caps(mgtfile)
          call caps(solfile)
          call caps(chmfile)
          ...
          if (septfile /='             ') then
            call caps (septfile)
            open (septfile_num,file=septfile, status='old')
            isep_hru(ihru) = 1
            call readsepticbz
          end if
          if (sdrfile /= '             ') then
	          call caps(sdrfile)
	          open (sdrfile_num,file=sdrfile)
	          call readsdr
              if (sdrain(ihru) <= 0.) sdrain(ihru) = sdrain_bsn  !!671 4/5/19  nbs
          else
 ...
 !           sdrain(ihru) = 0.      !!!! nbs 11/25/15
          ...
          open (chmfile_num,file=chmfile)
          open (solfile_num,file=solfile)
          open (hrufile_num,file=hrufile)
          !open (109,file=mgtfile)
          open (mgtfile_num,file=mgtfile)
          open (gwfile_num,file=gwfile)
          call readhru
          call readchm
...
!!    set default values
      ...
      if (harg_petco(i) < 1.e-6) harg_petco(i) = 0.0023           
...
!!    check elevation band fractions
      sumebfr = 0.  !total area of subbasin modeled in elevation bands
      do j = 1, 10
        sumebfr = sumebfr + elevb_fr(j,i)
      end do
      if (sumebfr > 1.e-5) then
        if (sumebfr < .99) write (input_std_num,1000) i
      end if
