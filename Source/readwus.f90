...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do
      read (wusfile_num,5300,iostat=eof) titldum
      if (eof < 0) exit
      read (wusfile_num,5300,iostat=eof) titldum
      if (eof < 0) exit
      read (wusfile_num,5300,iostat=eof) titldum
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (swupnd(mon),mon = 1,6)
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (swupnd(mon),mon = 7,12)
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (wurch(mon,i),mon = 1,6)
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (wurch(mon,i),mon = 7,12)
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (swush(mon),mon = 1,6)
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (swush(mon),mon = 7,12)
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (swudp(mon),mon = 1,6)
      if (eof < 0) exit
      read (wusfile_num,5100,iostat=eof) (swudp(mon),mon = 7,12)
      exit
      end do
