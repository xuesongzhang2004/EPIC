...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, l, isb
      real :: t_ch, scmx, xx, rto, ql, sumq, tb, tp, xi, q
      integer :: int
      real :: max
      real :: a, b, c, d
...
!!    compute time of concentration (sum of overland and channel times)
        t_ch = 0
        ch_l1(j) = ch_l1(j) * hru_dafr(j) / sub_fr(hru_sub(j))                              !!  Junyu Qi comments: dividing chl1 into  number of HRU in subbasin segments
        t_ov(j) = .0556 * (slsubbsn(j)*ov_n(j)) ** .6 / hru_slp(j) ** .3
        t_ch = .62 * ch_l1(j) * ch_n(1,hru_sub(j)) ** .75 / ((da_km*hru_dafr(j))**.125*ch_s(1,hru_sub(j))**.375)
        tconc(j) = t_ov(j) + t_ch
...
!!    compute unit hydrograph for computing subbasin hydrograph from direct runoff
      ...
      end if                             !!R666b 7/19/17 nbs
      
      !compute the number of hydrograph points for flood routing Jaehak 2017  !!R666b 7/19/17 nbs
        do j = 1, nhru
