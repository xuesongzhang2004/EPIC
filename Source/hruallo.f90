...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: ap_ai, ap_af, mgt_op, ap_cf, ap_cc, ap_ci, jj, kk
      integer :: iopera_sub, mcri, ilnds
      real :: depth(25)
      real :: max
      ...
      do j= mhru1, mhru
          ...
          solfile = ""
          chmfile = ""
          read (subfile_num,5300,iostat=eof) hrufile, mgtfile, solfile, chmfile, ilnds
          if (eof < 0) return
          if (ilnds > 0) then 
            ils_nofig = 1
          end if      
          ...
          call caps(solfile)
          call caps(chmfile)
          open (solfile_num,file=solfile,recl=350)
          !! calculate # of soil layers in profile
          depth = 0.
          lyrtot = 0
          read (solfile_num,6000) titldum
          read (solfile_num,6000) titldum
          read (solfile_num,6000) titldum
          read (solfile_num,6000) titldum
          read (solfile_num,6000) titldum
          read (solfile_num,6000) titldum
          read (solfile_num,6000) titldum
          read (solfile_num,6100) (depth(k), k = 1, 25)
          do k = 1, 25
          ...
          open (mgtfile1_num,file=mgtfile)
      
    !!  calculate max number of operations per hru
          iopera_sub = 1
          mcri = 0
          do kk = 1, 30
              read (mgtfile1_num,6000) titldum
          end do
        
          do kk = 1, 1000
              read (mgtfile1_num,6300,iostat=eof) mgt_op, mgt1i
              if (eof < 0) exit
              if (mgt_op == 1) then
                mcri = mcri + 1
              end if
          ...
          close (mgtfile1_num)            !!   nubz test
                   
          open (chmfile1_num,file=chmfile)
          eof = 0
          do 
                do k = 1, 11
                  read (chmfile1_num,6000,iostat=eof) titldum
                  if (eof < 0) exit
                end do
                if (eof < 0) exit
                do
                  pstnum = 0
                  read (chmfile1_num,*,iostat=eof) pstnum
                  if (eof < 0) exit
