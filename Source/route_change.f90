...
!! route water through reach
      if (ievent_rch == 0) then
        ...
        if (irte == 1) call rtmusk !rtmusk_change_Zh_lateral !rtmusk_change_Zh_lateral !rtmusk_change_Zh   
      else
        if (irte == 0) call rthvsc                         !!R666 7/19/17 nbs   commented for now.    
...
!! compute contribution of water in bank storage to streamflow
      ...
      if (ievent_rch > 0) then
        do ii = 1, nstep_rch
          hrtwtr(ii) = hrtwtr(ii) + qdbank / real(nstep_rch)
          hsdti(ii) = max(0.,hrtwtr(ii) / (dthy * 3600.)) !m3 -> m3/s  !!R672 6/19/19/nbs
        end do
      end if
...
!! do not perform sediment routing for headwater subbasins
	!! when i_subhw = 0
	  ...
	  if (i_subhw == 0 .and. inum1 == inum2) then  !! do not perform sediment routing for headwater subbasins 
          if (ievent_rch == 0) then
        ...
        else            !! perform sediment routing for headwater subbasins  
            if (ievent_rch == 0) then
              ...
              do ii = 1, nstep_rch
               if (hrtwtr(ii) > 0. .and. hdepth(ii) > 0.) then
                 sedrch = sedrch + hsedyld(ii)   !! accumulated sediment transported out|metric tons  
                 rch_sil = rch_sil + hsedyld(ii)  !!All are assumed to be silt type particles
               end if
              end do
