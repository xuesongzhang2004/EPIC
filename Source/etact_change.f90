...
!!    real, parameter :: esd = 500., etco = 0.80, effnup = 0.1
      ...
      real :: sumsnoeb, evzp, eosl, dep, evz, sev, t1, t2
      real, external :: expo
      ...
      if (pet > 1.0e-6) then
        ...
        if (sno_hru(j) >= 0.5) then
          eaj = 0.5
	          ...
	          eaj = Exp(cej * (sol_rsd(1,j)+ 0.1))  !!Changed by Qichun to only account for residue effects on ET
        end if
        es_max = pet * eaj
...
!        if (pot_vol(j) > 1.e-4) es_max = 0.
        ...
        !!if (pet_day < es_max + ep_max) then
          ...
          if (pet < es_max + ep_max) then
            ...
            t1 = pet * es_max / (es_max + ep_max)
            !ep_max = pet * ep_max / (es_max + ep_max)
            t2 = pet * ep_max / (es_max + ep_max)
            es_max = t1
            ep_max = t2
          end if
          if (pet < es_max + ep_max) then
