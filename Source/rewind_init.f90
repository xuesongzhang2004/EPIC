...
!! files listed in .fig
       do idum = 1, mhyd
        ...
        select case (icode)
          case (6)   !!rechour
            rewind (unit=hour_in_num+inum1)
              do ii = 1, 6
                read (hour_in_num+inum1,5000) titldum
              end do
          case (10)  !!recday
            rewind (unit=day_in_num+inum1)
              do ii = 1, 6
                read (day_in_num+inum1,5000) titldum
              end do
          !!case (14) !!saveconc
          case default
        end select
...
!! weather files
       ...
       do idum = 1, nrgage
        if (rfile(idum) /= '             ') then
          rewind (unit=rfile_num+idum)
          do ii = 1, 4
            read (rfile_num+idum,5000) titldum
          end do
        end if
       end do
       !! temperature
       do idum = 1, ntgage
        if (tfile(idum) /= '             ') then
          rewind (unit=tfile_num+idum)
          do ii = 1, 4
            read (tfile_num+idum,5000) titldum
          end do
        end if
       end do
       !! solar radiation
       if (slrfile /= '             ') then
         rewind (unit=slrfile_num)
         read (slrfile_num,5000) titldum
       end if
       !! relative humidity
       if (rhfile /= '             ') then
         rewind (unit=rhfile_num)
         read (rhfile_num,5000) titldum
       end if
       !! wind speed
       if (wndfile /= '             ') then
         rewind (unit=wndfile_num)
         read (wndfile_num,5000) titldum
       end if
       !! PET
       if (petfile /= '             ') then
         rewind (unit=petfile_num)
