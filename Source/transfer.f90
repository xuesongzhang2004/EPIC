...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: k, ii
      real :: volum, tranmx, ratio, ratio1, nhyd_tr, xx
      
      nhyd_tr = ih_tran(inum5)                  !!R671 4/5/19  nbs
      vartran(:,inum3) = 0.                     !!R671 4/5/19  nbs
...
!! compute volume of water in source
      volum = 0.
      if (ihout == 2) then
        volum = res_vol(inum1) + varoute(2,nhyd_tr)   !!R671 4/5/19  nbs
      else
...
!        end select
        ...
        !! source is a reservoir                    !!R671 4/5/19  nbs
        if (ihout == 2) then
          !res_vol(inum1) = res_vol(inum1) - tranmx !!R671 4/5/19  nbs
          ratio = 1. - tranmx / volum               !!R671 4/5/19  nbs
          ratio1 = 1.- ratio                        !!R671 4/5/19  nbs
          res_vol(inum1) = res_vol(inum1) * ratio      !!R671 4/5/19  nbs    !!|m^3 H2O       |water
          res_nh3(inum1) = res_nh3(inum1) * ratio      !!R671 4/5/19  nbs    !!|kg N          |amount of ammonia in reservoir
          res_no2(inum1) = res_no2(inum1) * ratio      !!R671 4/5/19  nbs    !!|kg N          |amount of nitrite in reservoir
          res_no3(inum1) = res_no3(inum1) * ratio      !!R671 4/5/19  nbs    !!|kg N          |amount of nitrate in reservoir
          res_orgn(inum1)= res_orgn(inum1)* ratio      !!R671 4/5/19  nbs    !!|kg N          |amount of organic N in reservoir
          res_orgp(inum1)= res_orgp(inum1)* ratio      !!R671 4/5/19  nbs    !!|kg P          |amount of organic P in reservoir
          res_solp(inum1)= res_solp(inum1)* ratio      !!R671 4/5/19  nbs    !!|kg P          |amount of soluble P in reservior
          res_chla(inum1)= res_chla(inum1)* ratio      !!R671 4/5/19  nbs    !!|kg chl-a      |amount of chlorophyll-a leaving reaservoir
          do ii = 2, mvaro                              !!R671 4/5/19  nbs                          
              varoute(ii,nhyd_tr) = varoute(ii,nhyd_tr) * ratio   !!R671 4/5/19  nbs  
          end do                                        !!R671 4/5/19  nbs

          !!save vartran to add in rchinit/resinit 
          vartran(2,inum3) = tranmx                     !!R671 4/5/19  nbs
          vartran(3,inum3) = res_sed(inum1) * tranmx    !!R671 4/5/19  nbs
          vartran(4,inum3) = (res_orgn(inum1) + varoute(4,nhyd_tr))  & !!R671 4/5/19  nbs
                             / ratio * ratio1 
          vartran(5,inum3) = (res_orgp(inum1) + varoute(5,nhyd_tr))  & !!R671 4/5/19  nbs   
                             / ratio * ratio1 
          vartran(6,inum3) = (res_no3(inum1) + varoute(6,nhyd_tr))   &!!R671 4/5/19  nbs
                             / ratio * ratio1 
          vartran(7,inum3) = (res_solp(inum1) + varoute(7,nhyd_tr))  & !!R671 4/5/19  nbs
                             / ratio * ratio1 

          vartran(11,inum3) = lkpst_conc(inum1) * tranmx  !mg pesticide !!R671 4/5/19  nbs 
          vartran(13,inum3) = (res_chla(inum1) + varoute(13,nhyd_tr)) &    !!R671 4/5/19  nbs   
                             / ratio * ratio1                 
        else
        !! source is a reach                                    !!R671 4/5/19  nbs
          xx = tranmx
...
!          end if
          ...
          !vartran(2,inum3) = varoute(2,nhyd_tr) / ratio * ratio1   !!R682 10/20/21 nbs
          !!save vartran to add in rchinit                          !!R682 10/20/21 nbs
          if (ratio < 0.01) then                                    !!R682 10/20/21 nbs
            vartran(2,inum3) = 0.                                   !!R682 10/20/21 nbs
            ratio1 = 0.                                             !!R682 10/20/21 nbs
