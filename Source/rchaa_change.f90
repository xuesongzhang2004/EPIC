...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
 ...
 !! added NO3 Concentration to output.rch (for daily only) gsm 10/26/2011
        pdvar(47) = rchaao(63,j)              !!RPOC in (kg C)
        pdvar(48) = rchaao(64,j)              !!RPOC out (kg C)
        pdvar(49) = rchaao(65,j)              !!LPOC in (kg C)
        pdvar(50) = rchaao(66,j)              !!LPOC in (kg C)
        pdvar(51) = rchaao(67,j)              !!RDOC in (kg C)
        pdvar(52) = rchaao(68,j)              !!RDOC out (kg C)
        pdvar(53) = rchaao(69,j)              !!LDOC in (kg C)
        pdvar(54) = rchaao(70,j)              !!LDOC in (kg C)
        pdvar(55) = rchaao(71,j)              !!DIC in (kg C))
        pdvar(56) = rchaao(72,j)              !!DIC out (kg C))
        pdvar(57) = rchaao(64,j)+rchaao(66,j) !!Total POC out (kg C)
        pdvar(58) = rchaao(68,j)+rchaao(70,j) !!Total DOC out (kg C)
        pdvar(59) = rchaao(64,j)+rchaao(66,j) + rchaao(68,j)+rchaao(70,j) !!Total DOC out (kg C)
        ...
        if (ipdvar(1) > 0) then
          ...
          write (output_rch_num,5000) j, subgis(j), years, rch_dakm(j), &
	  		(pdvr(ii), ii = 1, itotr)
...
!!  increase to 44 in loops below from 42 gsm 10/17/2011
      ...
      return
 5000 format ('REACH ',i5,1x,i8,1x,f5.1,47e12.4,13e12.4)        !!R666b 7/19/17 nbs    
 6000 format ('REACH ',i5,1x,i8,1x,f5.1,47e12.4,1x,i4)          !!R666b 7/19/17 nbs
      end