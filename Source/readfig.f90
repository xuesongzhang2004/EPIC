...
!!    initialize variables
      ...
      do 
        a = ""
        read (figfile2_num,5002,iostat=eof) a
        if (eof < 0) exit
        if (a /= "*") then
          backspace figfile2_num
          idum = idum + 1

!!    CEAP project
        if (isproj == 2) then
          read (figfile2_num,5003) a, icodes(idum), ihouts(idum), inum1s(idum),  &
         inum2s(idum), inum3s(idum), rnum1s(idum), inum4s(idum)
	  else !5000 format (a1,9x,5i6,f6.3,i9,i3,3a3)
	       !                10, 40,  46,55,58, 67
          read (figfile2_num,5000) a, icodes(idum), ihouts(idum), inum1s(idum),  &
         inum2s(idum), inum3s(idum), rnum1s(idum), inum4s(idum),    &    
...
!!!!!! end convert code         
          ...
          select case(icodes(idum))
            ...
            case (1)  !! icode = 1  SUBBASIN command
              ...
              subfile = ""
              read (figfile2_num,5100) subfile !5100 format (10x,2a13)
              call caps(subfile)
              i = 0
              i = inum1s(idum)
              subed(ihouts(idum)) = inum4s(idum)
              open (subfile2_num,file=subfile)
              call readsub
              nhru = nhru + hrutot(i)
...
!!            assume subbasin is the same number as the reach (if zero)              
              ...
              rtefile = ""
              swqfile = ""
              read (figfile2_num,5100) rtefile, swqfile
              call caps(rtefile)
              call caps(swqfile)
              irch = 0
              irch = inum1s(idum)
              open (rtefile_num,file=rtefile)
              open (swqfile_num,file=swqfile)
              call readrte
              call readswq
              close (swqfile_num)
              close (rtefile_num)
            ...
            case (3)  !! icode = 3  ROUTE RESERVOIR command
              ...
              resfile = ""
              lwqfile = ""
              read (figfile2_num,5100) resfile, lwqfile
              call caps(resfile)
              call caps(lwqfile)
              i = 0
              i = inum1s(idum)
              open (resfile_num,file=resfile)
              call readres
              close(resfile_num)
              if (lwqfile /= '             ') then
                open (lwqfile_num,file=lwqfile)
                call readlwq
                close (lwqfile_num)
              end if
            ...
            case (6)  !! icode = 6  RECHOUR command: read in hourly values
              ...
              hour_in = ""
              read (figfile2_num,5100) hour_in
              call caps(hour_in)
              open (hour_in_num+inum1s(idum),file=hour_in,recl=350)
              do ii = 1, 6
                read (hour_in_num+inum1s(idum),5200) titldum
              end do

            case (7)  !! icode = 7  RECMON command:
                      !!  read in monthly values
            month_in = ""
            read (figfile2_num,5100) month_in
              recmonps(ihouts(idum)) = month_in(1:index(month_in,'.')-1)
            call caps(month_in)
            i = 0
            i = inum1s(idum)
            open (month_in_num,file=month_in,recl=350)
            call readmon
            close (month_in_num)
            ...
            year_in = ""
            read (figfile2_num,5100) year_in
            
            call caps(year_in)
            i = 0
            i = inum1s(idum)
            open (year_in_num,file=year_in,recl=350)
            call readyr
            ...
            case (9)  !! icode = 9  SAVE command: saves daily reach
              ...
              day_in = ""
              read (figfile2_num,5100) day_in
              call caps(day_in)
              if (inum1s(idum) <= 10 .and. inum1s(idum) > 0) then
                open (day_save_num+inum1s(idum),file=day_in,recl=350)
                if (inum3s(idum) == 0) then
                  write (day_save_num+inum1s(idum),5400) title
                  write (day_save_num+inum1s(idum),5500)
                else
                  iida = 0
                  iida = idaf
                  call xmon
                  write (day_save_num+inum1s(idum),5501) iyr, i_mo, &
		  				(iida - ndays(i_mo))
                end if
              end if
            ...
            case (10) !! icode = 10  RECDAY command: read in daily values
              ...
              read (figfile2_num,5100) day_in
              call caps(day_in)
              open (day_in_num+inum1s(idum),file=day_in,recl=350)
              do ii = 1, 6
                read (day_in_num+inum1s(idum),5200) titldum
              end do
            ...
            case (11) !! icode = 11  RECCNST command: read in average 
                      ...
                      !! the nutrients in kg
            annual_in = ""
            read (figfile2_num,5100) annual_in
            reccnstps(ihouts(idum))=annual_in(1:index(annual_in,'.')-1)
            call caps(annual_in)
            i = 0
            i = inum1s(idum)
            open (annual_in_num,file=annual_in,recl=350)
            call readcnst
            close (annual_in_num)
...
!! code to read from apex output file
            case (13) 
            apex_in = ""
            read (figfile2_num,5100) apex_in
	      call caps(apex_in)
      !      i = 0
      !      i = inum1s(idum)
            open (apex_in_num+inum1s(idum),file=apex_in,recl=350)
	      do ii = 1, 10                                 !!R667 8/14/17 nbs
              read (apex_in_num+inum1s(idum),5200) titldum
            end do
!! code to read from apex output file
            ...
            case (17)  !! icode = 17  ROUTING UNIT command
              rufile = ""
              read (figfile2_num,5100) rufile
              call caps(rufile)
              iru = inum1s(idum)
              isub = inum2s(idum)
              daru_km(isub,iru) = rnum1s(idum)
              open (rufile_num,file=rufile)
              call readru
              close(rufile_num)
