...
!! determine amount of mineral N to be applied
      if (strsn(j) < auto_nstrs(j)) then
        targn = 0.                               !!targn       |kg N/ha       |target mineral N application
        if (nstress(j) == 0) then                !! n target approach
             tsno3 = 0.                              !!total soil mineral N
             tpno3 = 0.                              !!total plan N
             do ly = 1, sol_nly(j)
               tsno3 = tsno3 + sol_no3(ly,j) + sol_nh4(ly,j)
             end do
             tpno3 = plantn(j)

             targn = tnylda(j) - tsno3 - tpno3       !!tnylda(:)   |kg N/kg yield |estimated/target nitrogen content of yield, used in autofertilization
             if (targn > auto_napp(j)) targn = auto_napp(j) !!auto_napp(:)|kg NO3-N/ha   |maximum NO3-N content allowed in one fertilizer application
             if (targn < 0.) targn = 0.

             anano3(j) = anano3(j) + targn
             if (anano3(j) >= auto_nyr(j)) then         !!auto_nyr(:) |kg NO3-N/ha   |maximum NO3-N content allowed to be applied in one year by auto-fertilization
              ...
              targn = auto_napp(j) * (1. - phuacc(j))   !!auto_napp(:)|kg NO3-N/ha   |maximum NO3-N content allowed in one fertilizer application
...
!!          dwfert = 0.
        ...
        do ly = 1, 2
           ...
           !! ///added by Junyu Qi 04/2021; Zhang corrected on June 30, 2022
            if(ifrt == 4) then
                Inhibday(j) = 0   !! =1 for simulating inhib.
                sol_urea(ly,j)= sol_urea(ly,j)+ xx * frt_kg *  fnh3n(ifrt) * fminn(ifrt)
                sol_ph(ly,j)= 8.  !! urea application
            else
            !sol_ph(k,j)= ori_sol_ph(k,j)
                sol_no3(ly,j) = sol_no3(ly,j) + xx * dwfert *  fminn(ifrt) * (1. - fnh3n(ifrt))
                sol_nh4(ly,j) = sol_nh4(ly,j) + xx * dwfert * fnh3n(ifrt) * fminn(ifrt)
                sol_solp(ly,j) = sol_solp(ly,j) + xx * dwfert  * fminp(ifrt)
                
                no3_fert(j) = no3_fert(j) + xx * dwfert * fminn(ifrt) *  (1. - fnh3n(ifrt))
                nh4_fert(j) = nh4_fert(j) + xx * dwfert * fminn(ifrt) * fnh3n(ifrt)
            end if
           !! added by Junyu Qi 04/2021; Zhang corrected on June 30, 2022  ///
  
            no3_autof(j) = no3_autof(j) + xx * dwfert * fminn(ifrt) *  (1. - fnh3n(ifrt))
            nh4_autof(j) = nh4_autof(j) + xx * dwfert * fminn(ifrt) * fnh3n(ifrt)
            solp_autof(j) = solp_autof(j)+ xx * dwfert  * fminp(ifrt)
                        
            if (cswat == 0) then
		        sol_fon(ly,j) = sol_fon(ly,j) + rtoaf * xx * dwfert * forgn(ifrt)
                sol_aorgn(ly,j) = sol_aorgn(ly,j) + (1. - rtoaf) * xx * dwfert * forgn(ifrt)
                sol_fop(ly,j) = sol_fop(ly,j) + rtoaf * xx * dwfert * forgp(ifrt)
                sol_orgp(ly,j) = sol_orgp(ly,j) + (1. - rtoaf) * xx * dwfert* forgp(ifrt)
	        end if
	        if (cswat == 1) then
	        ...
	        if (cswat == 2) then
                ...
                orgn_autof(j) = orgn_autof(j) + xx * dwfert * forgn(ifrt)         
					 
                orgp_autof(j) = orgp_autof(j) + xx * dwfert * forgp(ifrt)         
                ...
                if (forgn(ifrt)  .gt. 1.E-6) then  
                    ...
                    OrgC_Fer(j)= OrgC_Fer(j)+ xx*dwfert*orgc_f             
                    orgn_fert(j)=orgn_fert(j)+ xx*dwfert*forgn(ifrt)       
                    ...
                    !sol_LMN(ly,j) = sol_LMN(ly,j) + X1 *rtoaf *forgn(ifrt) * X10
                    !sol_LSN(ly,j) = sol_LSN(ly,j) + X1 *rtoaf *forgn(ifrt) *(1- X10)
                    sol_LMN(ly,j) = sol_LMN(ly,j) + X1 *forgn(ifrt) * X10
                    sol_LSN(ly,j) = sol_LSN(ly,j) + X1 *forgn(ifrt) *(1- X10)
                    ...
                    CFOrfSTR(ly,j) = XZ
                    CFOrfMET(ly,j) = XXX
                    NFPltSTR(ly,j) = X1*rtoaf *forgn(ifrt) *(1- X10)
                    NFPltMET(ly,j) = X1 *rtoaf *forgn(ifrt) * X10
...
!!        end if
          ...
          solp_autof(j) = solp_autof(j)+ xx * dwfert * tfp           
...
!!        autop = autop + dwfert *(tfp + forgp(ifrt))
          ...
          tautop(j) = tautop(j) + autop     !!    tautop(:)   |kg P/ha       |amount of P applied in autofert operation in year
        if (curyr > nyskip) then
             wshd_ftotn = wshd_ftotn + dwfert * (fminn(ifrt) +  forgn(ifrt))* hru_dafr(j)
             wshd_forgn = wshd_forgn + dwfert * forgn(ifrt) * hru_dafr(j)   
             wshd_fno3 = wshd_fno3 + dwfert * fminn(ifrt) * (1. - fnh3n(ifrt)) * hru_dafr(j)
