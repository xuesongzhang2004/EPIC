...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      rhmeas = 0.
        ...
        !! read relative humidity data from file
          if (ifirsth == 0) then
            read (rhfile_num,5200) (rhmeas(l), l = 1, nhtot)
          else
            ifirsth = 0
            do
              iyp = 0
              idap = 0
              read (rhfile_num,5300) iyp, idap, (rhmeas(l), l = 1, nhtot)
              if (iyp + idap <= 0) exit
	    ...
	    !! generate values to replace missing data
          ...
          !! use same generated data for all HRUs in a subbasin
           ...
           if (rhmeas(ihgage(hru_sub(k))) > 1. .or.            &     !!R682 10/20/21 nbs                  
                         rhmeas(ihgage(hru_sub(k))) < 0.) then     !!R6!!R682 10/20/21 nbs82 10/20/21 nbs
             !! convert dew point to humidity                       
             tmpmean=(tmpmx(i_mo,hru_sub(k))+tmpmn(i_mo,hru_sub(k)))/2. !!R682 10/20/21 nbs
             rhd(k) = Ee(rhmeas(ihgage(hru_sub(k)))) / Ee(tmpmean)      !!R682 10/20/21 nbs
           else                                                     !!R682 10/20/21 nbs
             rhd(k) = rhmeas(ihgage(hru_sub(k)))                    !!R682 10/20/21 nbs                    
           endif                                                    !!R682 10/20/21 nbs                                                  
        end if                                                      !!R682 10/20/21 nbs
	  end do

      return

 5200 format (7x,1900f8.3)              !!R681 5/26/20 nbs
 5300 format (i4,i3,1900f8.3)           !!R681 5/26/20 nbs
      end