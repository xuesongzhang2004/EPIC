...
!     skipping hourly bacteria route for now  04/16/07 nubs
      if (ievent_rch > 0) then                !! hourly mass balance
        initlp = 0.
        initp = 0.
        ...
        do ii = 1, nstep_rch
          ...
          totbactp = 0.
          totbactlp = 0.
          totbactp = hhvaroute(18,inum2,ii) * hhvaroute(2,inum2,ii) *  (1. - rnum1) + initp * hrchwtr(ii)
          totbactlp = hhvaroute(19,inum2,ii) * hhvaroute(2,inum2,ii) *  (1. - rnum1) + initlp * hrchwtr(ii)
	  ...
	  if (totbactlp < 1.e-6) totbactlp = 0.0
        if (netwtr >= 1.) then
          rch_bactp(jrch) = hbactp(nstep_rch)
          rch_bactlp(jrch) = hbactlp(nstep_rch)
        else
          rch_bactp(jrch) = 0.
          rch_bactlp(jrch) = 0.
        end if
...
!! daily mass balance
      ...
      totbactp = 0.
      totbactlp = 0.
      totbactp = varoute(18,inum2) * varoute(2,inum2) * (1. - rnum1)  + rch_bactp(jrch) * rchwtr
      totbactlp = varoute(19,inum2) * varoute(2,inum2) *  (1. - rnum1) + rch_bactlp(jrch) * rchwtr
