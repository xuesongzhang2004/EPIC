...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      open (file_cio2_num,file="file.cio")

!! Read project description
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,5100) title

!! Read general information/watershed configuration
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,5000) figfile
      read (file_cio2_num,*) nbyr
      read (file_cio2_num,*) iyr
      read (file_cio2_num,*) idaf
      read (file_cio2_num,*) idal

      call caps(figfile)
      open (figfile2_num,file=figfile)

!! Read climate information
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,*) igen
      read (file_cio2_num,*) pcpsim
      read (file_cio2_num,*) idt
      read (file_cio2_num,*) idist
      read (file_cio2_num,*) rexp
      read (file_cio2_num,*) nrgage
      read (file_cio2_num,*) nrtot
      read (file_cio2_num,*) nrgfil
      read (file_cio2_num,*) tmpsim
      read (file_cio2_num,*) ntgage
      read (file_cio2_num,*) nttot
      read (file_cio2_num,*) ntgfil
      read (file_cio2_num,*) slrsim
      read (file_cio2_num,*) nstot
      read (file_cio2_num,*) rhsim
      read (file_cio2_num,*) nhtot
      read (file_cio2_num,*) wndsim
      read (file_cio2_num,*) nwtot
      read (file_cio2_num,*) fcstyr
      read (file_cio2_num,*) fcstday
      read (file_cio2_num,*) fcstcycles
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,5000) (rfile(j),j = 1,18)
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,5000) (tfile(j),j = 1,18)
      read (file_cio2_num,5000) slrfile
      read (file_cio2_num,5000) rhfile
      read (file_cio2_num,5000) wndfile
      read (file_cio2_num,5000) fcstfile
...
!     & Soil Layers')
      ...
      call gcycl        !!!!    This subroutine initializes the random number seeds.
      ...
      call caps(fcstfile)
      if (fcstfile /= '             ') then 
        open (fcstfile2_num,file=fcstfile)
      else
        fcstyr = 0
        fcstday = 0
      end if
...
!!Open watershed modeling option file
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,5000) bsnfile
...
!!Open database files
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,5000) plantdb
      read (file_cio2_num,5000) tilldb
      read (file_cio2_num,5000) pestdb
      read (file_cio2_num,5000) fertdb
      read (file_cio2_num,5000) urbandb
      ...
      call caps(fertdb)
      call caps(urbandb)
      open (plantdb2_num,file=plantdb)
      open (tilldb2_num,file=tilldb)
      open (pestdb2_num,file=pestdb)
      open (fertdb2_num,file=fertdb)
      open (urbandb2_num,file=urbandb)
...
!!Output Information input
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,*) iprint
      read (file_cio2_num,*) nyskip
!!! check that nyskip input is not greater or equal to nbyr input
      if (nyskip >= nbyr) nyskip = nbyr - 1
      
      read (file_cio2_num,*) ilog
      read (file_cio2_num,*) iprp
      read (file_cio2_num,5101) titldum
      ...
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,*) (ipdvar(ii),ii=1,20)
      ...
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,*) (ipdvab(ii),ii=1,15)
      ...
      read (file_cio2_num,5101) titldum
      read (file_cio2_num,*) (ipdvas(ii),ii=1,20)
      ...
      do
       read (file_cio2_num,5101,iostat=eof) titldum
       if (eof < 0) exit
...
!!   IPHR = 1 print file
      iphr = 0
      read (file_cio2_num,*,iostat=eof) iphr
!!   code for printing soil storage values by soil layer (output.swr)
!!   ISTO = 0 no print
!!   ISTO = 1 print file
      isto = 0
      read (file_cio2_num,*,iostat=eof) isto
...
!!    output by elevation band to (formerly 'snowband.out')
      ...
      isnow = 0
	if (isnow == 1) then
         open (output_snw_num,file='output.snw')
         write (output_snw_num,1010)
!!    output temperatures by elevation band (Dhiraj Raj - 07/22/2015)
         open (ebandtemp_out_num,file='ebandtemp.out')
         write (ebandtemp_out_num,1011)
      end if
      ...
      do ii = 1, 20
        if (ipdvar(ii) > 0) itotr = itotr + 1           !!    ipdvar(:)   |none        |output variable codes for output.rch file
      end do
 ...
 !! change 42 to 45 for output.rch file gsm 10/30/2011     
      ...
      do ii = 1, 15
        if (ipdvab(ii) > 0) itotb = itotb + 1           !!    ipdvab(:)   |none        |output variable codes for output.sub file
      end do
      ...
      do ii = 1, 20
        if (ipdvas(ii) > 0) itots = itots + 1           !!    ipdvas(:)   |none        |output varaible codes for output.hru file
      end do
      ...
      if (ipdhru(1) <= 0) then              !|HRUs whose output information will be printed to the output.hru and output.wtr files
        do ii = 1, mhru
!       do ii = 1, mhruo
          ipdhru(ii) = ii
        end do
...
!       itoth = mhruo
      ...
      open (output_hru_num,file="output.hru",recl=2000)   
      if (ia_b == 1) then 
        open (outputb_hru_num,file="outputb.hru",form='unformatted')
      end if
      open (output_pst_num,file="output.pes",recl=600)      !!R682 10/20/21 nbs
      open (output_sub_num,file="output.sub",recl=1500)   
      if (ia_b == 1) then
        open (outputb_sub_num,file = "outputb.sub", form = 'unformatted')
      end if
      open (output_rch_num,file="output.rch",recl=1500)   
      open (output_rsv_num,file="output.rsv",recl=800)
      if (ia_b == 1) then
        open (outputb_rch_num,file = "outputb.rch", form = 'unformatted')
      end if
      
!!    sediment routing output file
      open (output_sed_num,file="output.sed",recl=800)
!! write headings to sediment outputfile (output.sed)
      write (output_sed_num,1080)
1080  format (t10,'RCH',t17,'GIS',t23,'MON',t31,'AREAkm2',             & !!R682 10/20/21 nbs
     ...
     'CH_BEDtons',t232,'CH_DEPtons',t244,'FP_DEPtons',t259,'TSSmg/L')
      ...
      write(bmp_sedfil_out_num,'(a46)') ''   ! 
       !retention-irrigation output
      open (bmp_ri_out_num,file = "bmp-ri.out") !jaehak temp urban print out
      write(bmp_ri_out_num,'(a46)') 'Retention-Irrigation Basins Configuration'
      write(bmp_ri_out_num,'(a46)') ''   ! 


!! srin output file from watqual.f  
      if (ihumus ==1) then
        open (output_wql_num,file='output.wql')
        write (output_wql_num,6000)
...
!! mauro/jerry whittaker hourly output file
      if (iphr > 0) then
        open (hourq_out_num,file='hourq.out')
        write (hourq_out_num,6001) 
...
!!darrell output files added for interface plotting
      open (rch_dat_num ,file='rch.dat')
      open (hru_dat_num,file='hru.dat')
      open (sub_dat_num,file='sub.dat')
...
!! Output daily streamflow velocity for each channel (subbasin)
      if (itemp == 1) then
         open (output_vel_num,file='output.vel')
         write (output_vel_num,4999)
 4999     format(t17,'CH_VEL',/,t3,'Day',t7,'Year',t18,'(m/s)')
         open (output_dep_num,file='output.dep')
          write (output_dep_num,4998)
...
!! septic result  J.Jeong Feb2009
      open (septic_out_num,file='septic.out')  
	write(septic_out_num,5102) 'HRU','YEAR','DAY','Precip', 'PERC',        &
      'sol_ul','sol_st','sol_fc','nh3bgn','nh3end',             &
      'no3bgn','no3end', 'nitrN','denitrN',                     &
      'solpbgn','solpend','solpconc'
      write(septic_out_num,5102) '#','','','(mm)','(mm)','(mm)',           &
      '(mm)','(mm)','(kg/ha)','(kg/ha)',                        &
