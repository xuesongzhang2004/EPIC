...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, l
      real :: rto, rmn1, roc, wetness, base
      real :: vara,varb,varc,as_p_coeff, arate, xx, ssp
	real  solp(mlyr),actp(mlyr),stap(mlyr) !! locals for concentation based data
...
!! If total P is greater than 10,000 mg/kg do not allow transformations at all
	   If ((solp(l) + actp(l) + stap(l)) < 10000.) then 
		  ...
		  sol_solp2minp(j)= sol_solp2minp(j)  + rmn1        
