...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: bioleaf, biomxtrees, extcoef, rsr1c, rsr2c, bmdieoff
...
!!      initialize locals in loop
        ...
        read (plantdb2_num,*,iostat=eof) ic, cname, idtype
        if (eof < 0) exit
        read (plantdb2_num,*,iostat=eof) bioe, hvstc, blaic, frgrw1, laimx1,  frgrw2, laimx2, dlaic, chtmxc, rdmxc
        if (eof < 0) exit
        read (plantdb2_num,*,iostat=eof) topt, tbase, cnyldc, cpyldc, bn1, bn2,  bn3, bp1c, bp2c, bp3c
        if (eof < 0) exit
        read (plantdb2_num,*,iostat=eof) wsyfc, usle_c, gsic, vpdfr, frgmax,  wavpc, co2hi, bioehi, rsdcopl, alaimin
        if (eof < 0) exit
        read (plantdb2_num,777,iostat=eof) bioleaf, yrsmat, biomxtrees, extcoef, bmdieoff, rsr1c, rsr2c
...
!!        fix bad input for pltpfr(3,ic)
          if (pltpfr(1,ic) - pltpfr(2,ic) < .0001)    pltpfr(2,ic) = pltpfr(1,ic) - .0001
          if (pltpfr(2,ic) - pltpfr(3,ic) < .0001)    pltpfr(3,ic) = .75 * pltpfr(3,ic)
          b1 = 0.0
          b2 = 0.0
          b3 = 0.0
