      subroutine bmp_det_pond               !R668 1/30/2018 nbs
...
!	   if (qdepth<=0.0001) cycle   
         ...
         if (dtp_stagdis(sb)==0) then 
	        ...
	        !! Circular weir
               ...
               !! Fully submerged 
		    	...
		    	sqrt(19.6 * watdepact) !m3/s/unit
                  qstage = qstage * dtp_numweir(sb) * 60. * idt_rch !m^3
                 else
               !! Partially submerged
          ...
          if (dtp_weirtype(sb,k)==1 .and. watdepact>0.) then
            qstage = dtp_cdis(sb,k) * 1.84 * 	&
                  dtp_totwrwid(sb) * (watdepact ** 1.5) * 60. * idt_rch !m3/s
            qout = qout + qstage 
          end if
         ...
         if (qout>qpnd) then !no detention occurs
            ...
            seep_vol = 10.0 * ch_k(2,sb) * seep_sa * idt_rch / 60. !m^3
         ...
         write (outputb_hru_num,'(8f10.2,4f10.6)') qin,	&
      ...
      end subroutine bmp_det_pond           !R668 1/30/2018 nbs 