...
!!    changes for Mike Winchell if apex.swt files has a different start date than the SWAT run
      if (ifirsta(inum1) == 1) then
        do 
          read (apex_in_num+inum1,*) idapa(inum1), iypa(inum1), flodaya(inum1),     &
      ...
      if (ievent_rch > 0) then
        do ii = 1, nstep_rch
          hhvaroute(2,ihout,ii) = flodaya(inum1) / real(nstep_rch)
          hhvaroute(3,ihout,ii) = seddaya(inum1) / real(nstep_rch)
          hhvaroute(4,ihout,ii) = orgndaya(inum1) / real(nstep_rch)
          hhvaroute(5,ihout,ii) = orgpdaya(inum1) / real(nstep_rch)
          hhvaroute(6,ihout,ii) = no3daya(inum1) / real(nstep_rch)
          hhvaroute(7,ihout,ii) = minpdaya(inum1) / real(nstep_rch)
        end do
      end if
