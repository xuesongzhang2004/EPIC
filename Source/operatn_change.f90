...
!! operations performed only when no land cover growing
        ...
        do while(idop(nop(j),j) > 0 .and. iida == idop(nop(j),j) .and. iyr == mgtyr_hru(j))       !idop(iop,ihru) = Jdt(ndays,day,mon)
          
          call sched_mgt
          !if (mgtop(nop(j),j) == 17) then       !  simulate.f90(332):            if (mgtop(nop(j),j) /= 17) then
!simulate.f90(338):            nop(j) = nop(j) + 1
!simulate.f90(340):            if (nop(j) > nopmx(j)) then
!simulate.f90(341):              nop(j) = 1
!simulate.f90(347):          if (mgtop(nop(j),j) == 17) then
!simulate.f90(348):            nop(j) = nop(j) + 1
!simulate.f90(349):            if (mgtop(nop(j),j) == 17) then

!simulate.f90(265):               if (mgtop(nop(ihru),ihru) /=17) then         
!simulate.f90(270):               nop(ihru) = nop(ihru) + 1
!simulate.f90(272):                if (nop(ihru) > nopmx(ihru)) then
!simulate.f90(273):                  nop(ihru) = 1
        ...
        do while (phu_op(nop(j),j) > 0. .and. aphu > phu_op(nop(j),j) .and. iyr == mgtyr_hru(j))

          call sched_mgt            !nop(j) = nop(j) + 1
          if (igro(j) == 0) then
            aphu = phubase(j)
          else
            aphu = phuacc(j)
