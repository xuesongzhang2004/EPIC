...
!                                 all soil layers 
    ...
    !! private variables
      ...
      real ::  sol_mass, sol_cmass, sol_nmass
...
!!    zero new carbon variables for output.hru
      ...
      sol_mass = (sol_thick(k,j) / 1000.) * 10000. * sol_bd(k,j)  &
	...
	sum_n_i = sol_nmass + sol_no3(k,j) + sol_nh4(k,j) + sol_fon(k,j) &
			+ sol_mn(k,j)
...
!!		xx = sqrt(cdg * sut)
        ...
        !sol_orgp(k,j) = amin1(sol_orgp(k,j), 25.)  !commented 110118 Armen K !!R671 4/5/19  nbs
		...
		if ((sol_no3(k,j) + sol_nh4(k,j)) > 0.) then
			xx = (rnet_N + mnet_N) / (sol_no3(k,j) + sol_nh4(k,j))
            if (xx < -1.) then
                rdc = -rdc / xx
                rnet_N = -rnet_N / xx
                rnet_P = -rnet_P / xx
...
!! april 2010 output net_min
     ...
     &	!	sol_cdec * (1. / CNsoil)
		...
		if (rnet_N>0.) sol_nh4(k,j) = sol_nh4(k,j) + rnet_N 
		if (mnet_N>0.) sol_nh4(k,j) = sol_nh4(k,j) + mnet_N

		if (rnet_N<0.) then
			if (abs(rnet_N) < sol_nh4(k,j)) then
				sol_nh4(k,j) = sol_nh4(k,j) + rnet_N
			else
				xx4 = sol_nh4(k,j) + rnet_N
				sol_nh4(k,j) = 0.
		...
		if (mnet_N<0.) then
			if (abs(mnet_N) < sol_nh4(k,j)) then
				sol_nh4(k,j) = sol_nh4(k,j) + mnet_N
			else
				xx4 = sol_nh4(k,j) + mnet_N
				sol_nh4(k,j) = 0.
				sol_no3(k,j) = sol_no3(k,j) + xx4
			end if
		end if
	
		sol_nh4(k,j) = sol_nh4(k,j) + sol_cdec * (1. / CNsoil)
	...
	sum_n_f = sol_nmass + sol_no3(k,j) + sol_nh4(k,j) + sol_fon(k,j)    &
     	+ sol_mn(k,j) + wdn
	...
	!!if (i==365) then
!!	   write (cswat_profile_num,9000) iyr, i, k, j, sol_cmass, sol_cbn(k,j),
...
!!     &	sol_cmass/sol_nmass, sol_nmass/sol_orgp(k,j), sol_nh4(k,j),
...
!!    carbon outputs for .hru file
	...
	sol_nh3_pro = sol_nh3_pro + sol_nh4(k,j)
