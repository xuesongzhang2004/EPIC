...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
 	...
 	! sedimentation-filtration basin
         ...
         do kk=1,num_sf(sb)
           ...
           (iyr==sf_iy(sb,kk).and.i_mo>=sf_im(sb,kk))) then
               if(sf_typ(sb,kk)==2) then !partial scale 
                  call bmp_sand_filter(kk,sfflw,sfsed)  !R668 1/30/2018 nbs    
                  spqm3(:,:) = 0.
                  spsed(:,:)=0.
               ...
               elseif(sf_typ(sb,kk)==1) then !full scale
                  ...
                  call bmp_sed_pond(kk,sfflw,sfsed)         !R668 1/30/2018 nbs     
      ...
      write(bmp_sedfil_out_num,'(5i6,30f12.3)') iyr,iida,ii,sb,kk,sum(spqm3(1,:)),           &
      ...
      if(num_ri(sb)>=1.and.hrnopcp(sb,nstep)<96) then !72 hours draw-down plus another day (24hrs) 
         ...
         do kk=1,num_ri(sb)
           ...
           (iyr==ri_iy(sb,kk).and.i_mo>=ri_im(sb,kk))) then
               ...
               call bmp_ri_pond(kk,riflw,rised)          !R668 1/30/2018 nbs
      ...
      write(bmp_ri_out_num,'(5i6,30f12.3)') iyr,iida,ii,sb,kk,sum(riqm3(1,:)),           &
