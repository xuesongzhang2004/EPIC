...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !! compute snow melt
       if(isnowm==0) call snom     
      
       if(isnowm==1) call snom1     

      !! output by elevation band to output.snw
      if (isnow == 1) then    
        write(output_snw_num,1010) i, iyr, subnum(j), hruno(j), (snoeb(ib,j), ib = 1,10)
      end if
      
      if (isnow ==1) then
        write (ebandtemp_out_num,1010) i, iyr, subnum(j), hruno(j), (tavband(ib,j), ib = 1, 10)
      end if
      ...
      if (ri_luflg(j)==1) then                                      !ri_luflg(ihru) = 1 !irrigate HRU
      ...
      if (precipday > 0.1) then
        call volq                               !!surq_daycn or surq_greenampt
        !! bmp adjustment
      ...
      surfq(j) = surfq(j) + qird(j)     !aird(jj) = volmm * (1. - sq_rto), where sq_rto is the fraction of irrigation water that runs off.
      ...
      if (precipday > 0.01) call alph(0)        !al5         |none        |fraction of total rainfall on day that occurs during 0.5h highest intensity rainfall

      if (qday > 0.0001) then                   !qday          |mm H2O        |surface runoff loading to main channel for day in HRU
        !! compute peak rate - peakr in m3/s  
        call pkq(0)                             !peakr        |m^3/s         |peak runoff rate
                                                !as iwave = 0, HRU sediment calculations                                                
      end if  

      if (qday > 0.0001 .and. peakr > 0.) then
        !! compute transmission losses for non-HUMUS datasets
        call tran                               !!computes tributary channel transmission losses using daily time step 
        call eiusle                             !!computes the USLE erosion index (EI) using daily time step

	!! calculate sediment erosion by rainfall and overland flow
		call ovr_sed                            !!computes splash erosion by raindrop impact and flow erosion by overland flow using EUROSEM
      end if
