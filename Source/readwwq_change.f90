...
!!    initialize variables
      ...
      do
      read (wwqfile_num,5100,iostat=eof) titldum
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) lao 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) igropt 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) ai0 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) ai1 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) ai2 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) ai3 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) ai4
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) ai5 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) ai6 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) mumax 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) rhoq 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) tfact 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) k_l 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) k_n 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) k_p 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) lambda0 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) lambda1
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) lambda2 
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) p_n
      if (eof < 0) exit
      read (wwqfile_num,*,iostat=eof) chla_subco
      if (eof < 0) exit
      exit
      end do
...
!! change units from day to hour if hourly (subdaily) routing is performed
      ...
      if (ievent_rch > 0) then    
        mumax = mumax / real(nstep_rch)
        rhoq = rhoq / real(nstep_rch)
    !! convert units on k_l:read in as kJ/(m2*min), use as MJ/(m2*time step)
        k_l = k_l * 1.e-3 * idt_rch
        else 
