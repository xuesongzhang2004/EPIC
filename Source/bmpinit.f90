...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
   ...
   !!            stop
      ...
      do k=1,num_ri(i)
       ...
       write(bmp_ri_out_num,'(a11,i5)') 'Subbasin #:', i   ! bmp_sedfil.out
       write(bmp_ri_out_num,'(a46)') '' 
       write(bmp_ri_out_num,'(a10,i5)') 'RI #:', K   ! bmp_sedfil.out
       write(bmp_ri_out_num,'(a17,f10.1,a4)') 'Total volume =', ri_vol(i,k),'m^3'
       write(bmp_ri_out_num,'(a17,f10.1,a4)') 'Surface area =', ri_sa(i,k),'m^2'
       write(bmp_ri_out_num,'(a17,f10.1,a3)') 'Drawdown time =', ri_dd (i,k),'hr'
       write(bmp_ri_out_num,'(a17)') ''
      ...
      do k=1,num_sf(i)
         write(bmp_sedfil_out_num,'(a11,i5)') 'Subbasin #:', i   ! bmp_sedfil.out
         write(bmp_sedfil_out_num,'(a46)') '' 
         write(bmp_sedfil_out_num,'(a10,i5)') 'SED-FIL #:', K   ! bmp_sedfil.out
         !determine water quality volume for defult pond sizes
         ...
         if (sf_dim(i,k)==0) then
            write(bmp_sedfil_out_num,'(a46)') 'This SED-FIL size is automatically' &
...
!               wqv/(4.+1.33*4.) * 0.093
   ...
   !     if (ft_pd(i,k)>254) ft_pd(i,k) = 254. ! max 10inches
         ...
         if (sf_typ(i,k)==1) then
           write(bmp_sedfil_out_num,'(a37)') 'Full type sed-fil selected'
         elseif (sf_typ(i,k)==2) then
           write(bmp_sedfil_out_num,'(a40)') 'Partial type sed-fil selected'
         else
           write(bmp_sedfil_out_num,'(a43)') 'Sedimentation pond only selected'
         endif
         if (sf_typ(i,k)==1.or.sf_typ(i,k)==3) then     
      write(bmp_sedfil_out_num,'(a18)') 'Sedimentation pond'
      write(bmp_sedfil_out_num,'(a17,f10.1,a4)') 'Total volume =', sp_pvol(i,k),'m^3'
      write(bmp_sedfil_out_num,'(a17,f10.1,a4)') 'Surface area =', sp_sa(i,k),'m^2'
      write(bmp_sedfil_out_num,'(a17,f10.1,a3)') 'Drain Pipe Dia =', sp_pd(i,k),'mm'
      write(bmp_sedfil_out_num,'(a17)') ''
         endif
         if (sf_typ(i,k)==1.or.sf_typ(i,k)==2) then     
      write(bmp_sedfil_out_num,'(a11)') 'Sand Filter'
      write(bmp_sedfil_out_num,'(a17,f10.1,a4)') 'Surface area =', ft_sa(i,k),'m^2'
      write(bmp_sedfil_out_num,'(a17,f10.1,a3)') 'Max ponding =', ft_h(i,k),'mm'
      write(bmp_sedfil_out_num,'(a17,f10.1,a3)') 'Filter depth =', ft_dep(i,k),'mm'
      write(bmp_sedfil_out_num,'(a17,f10.1,a3)') 'Drain Pipe Dia =', ft_pd(i,k),'mm'
      write(bmp_sedfil_out_num,'(a17)') ''
        end if
        end do 
