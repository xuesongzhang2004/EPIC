...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: lid_cuminf_total, qvol, xx, por_air, d, yy, wat, swst_del, sw_del, wt_del, wtst_del, sumqtile
      real :: hru_mm                           !!R670_6/26/18  nbs 
      lid_cuminf_total = 0.
...
!          lid_cuminf_total = 0.
	  ...
	  if(j1==i_sep(j).and.sol_tmp(j1,j) > 0. .and. isep_opt(j) /= 0) then
		  !sol_st(j1,j) = sol_st(j1,j) + qstemm(j)  ! in mm              !!R670  6/26/18  nbs
          hru_mm = qstemm(j) * bz_area(j) / hru_ha(j) / 10000. !in mm    !!R670  6/26/18  nbs 
          sol_st(j1,j) = sol_st(j1,j) + hru_mm  ! in mm                  !!R670  6/26/18  nbs
	    qvol = qstemm(j) * hru_ha(j) * 10.
		  xx = qvol / hru_ha(j) / 1000.
          sol_no3(j1,j) = sol_no3(j1,j) + xx *(sptno3concs(isp) &
	  		+ sptno2concs(isp))  
          sol_nh4(j1,j) = sol_nh4(j1,j) + xx * sptnh4concs(isp) 
