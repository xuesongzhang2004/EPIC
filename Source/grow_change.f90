...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      rto = 1.
        ...
        if (iyr == 2014 .and. i == 205 .and. j == 1)  then
            print *, iyr, i, j, "STOP"
        end if
        ...
        if (idorm(j) == 1) return
        idp = idplt(j)              !!|land cover code from crop.dat
       ...
       !!------------------qichun co2----------------------------------------
          if (co2con(iyr) > 330.) then  
          
              if(ico2==1) then
                beadj = 100. * co2con(iyr) / (co2con(iyr) +   	&      
                       Exp(wac21(idp) - co2con(iyr)* wac22(idp)))    
	    ...
	    if (reg > 0.) then
         ...
         if (ifor==1)  call pweather_YANG  !! -------------------------qichun    foresty----------------------------------
         ...
         !! Eleminate effects of P stress on long term plant growth and C/N cycle
          if (reg < 0.) reg = 0.
         ! reg = Min(strsw(j), strstmp(j), strsn(j), strsp(j), strsa(j))
          ...
          !!============
          if (cswat == 2) then
            NPPC_d(j) = NPPC_d(j) + bioday * reg* CFB
          end if
          !!add by zhang
          !!============          
          ...
          f = 0.
          ff = 0.
          f = phuacc(j) / (phuacc(j) + Exp(leaf1(idp) - leaf2(idp) * phuacc(j)))      !equation 5:2.1.10
          ff = f - laimxfr(j)                                                         !equation 5:2.1.16 
          laimxfr(j) = f                                                              !f is current day laimxfr, and laimxfr is used to store previous day laimxfr
          !laimxfr is the fraction of the plant's maximum lean area index corresponding to a given fraction of potential heat units for the plant
