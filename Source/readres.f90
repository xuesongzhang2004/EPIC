...
!!    read in data from .res file
      do
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_sub(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) mores(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) iyres(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_esa(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_evol(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_psa(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_pvol(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_vol(i) 
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_sed(i) 
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_nsed(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_d50
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_k(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) iresco(i)
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (oflowmx(mon,i), mon = 1, 6)
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (oflowmx(mon,i), mon = 7, 12)
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (oflowmn(mon,i), mon = 1, 6)
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (oflowmn(mon,i), mon = 7, 12)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) res_rr(i)
        if (eof < 0) exit
        read (resfile_num,1100,iostat=eof) resmono
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) iflod1r(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) iflod2r(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) ndtargr(i)
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (starg(mon,i), mon = 1, 6)
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (starg(mon,i), mon = 7, 12)
        if (eof < 0) exit
        read (resfile_num,1100,iostat=eof) resdayo
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (wuresn(mon,i), mon = 1, 6)
        if (eof < 0) exit
        read (resfile_num,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) (wuresn(mon,i), mon = 7, 12)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) wurtnf(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) evrsv(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) oflowmn_fps(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) starg_fps(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) nostep
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) weirc(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) weirk(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) weirw(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) acoef(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) bcoef(i)
        if (eof < 0) exit
        read (resfile_num,*,iostat=eof) ccoef(i)
        if (eof < 0) exit
!! code added for C. Ikenberry to swicth from new (1) to old (0) equations in resnut.f
        read (resfile_num,*,iostat=eof) ires_nut
        if (eof < 0) exit
        exit
      end do
...
!!    set default values
      ...
      if (res_vol(i) < 0.0) res_vol(i) = 0.0            !!R682 10/20/21 nbs
...
!!    open daily reservoir outflow file 
      if (iresco(i) == 3) then
        call caps(resdayo)
        open (resdayo_num+i,file=resdayo)
        read (resdayo_num+i,1000) titldum
      end if
...
!! calculate sediment settling rate
	  ...
	  sed_stlr(i) = Exp(-.184 * res_d50 / nstep_rch)	!! urban modeling by J.Jeong
	endif   
...
!! read in monthly release data
      if (iresco(i) == 1) then
        call caps(resmono)
        open (resmono_num,file=resmono)
        read (resmono_num,1000) titldum
          do j = 1, nbyr+2
            read (resmono_num,*,iostat=eof) (res_out(i,mon,j), mon = 1, 12)
            if (eof < 0) exit
            do mon = 1, 12
