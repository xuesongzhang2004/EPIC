...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do j = 1, subtot
        ...
        pdvar(2) = srch_av(2)       !!flow out (m^3/s)               7 SWAT-CUP
        pdvar(3) = rchdy(3,j)       !!evaporation (m^3/s)            8
        pdvar(4) = rchdy(4,j)       !!tloss (m^3/s)                  9
        pdvar(5) = rchdy(5,j)       !!sed in (metric tons)           10
        pdvar(6) = rchdy(6,j)       !!sed out (met tons)             11
        pdvar(7) = rchdy(7,j)       !!sed conc (mg/L)                12
        pdvar(8) = rchdy(8,j)       !!orgN in (kg N)                 13
        pdvar(9) = rchdy(9,j)       !!orgN out (kg N)                14
        pdvar(10) = rchdy(10,j)     !!orgP in (kg P)                 15
        pdvar(11) = rchdy(11,j)     !!orgP out (kg P)                16
        pdvar(12) = rchdy(12,j)     !!NO3 in (kg N)                  17
        pdvar(13) = rchdy(13,j)     !!NO3 out (kg N)                 18
 ...
 !! added for NO3 Concentration to output.rch (daily only) gsm 10/26/2011
        ...
        pdvar(46) = rchdy(60,j)                      ! water temperature deg c                  !51   SWAT-CUP
         
  !----assign daily output values for carbon module,added by Du--------------!
        pdvar(47) = rchdy(63,j)             !!RPOC in (kg C)
        pdvar(48) = rchdy(64,j)             !!RPOC out (kg C)
        pdvar(49) = rchdy(65,j)             !!LPOC in (kg C)
        pdvar(50) = rchdy(66,j)             !!LPOC OUT (kg C)
        pdvar(51) = rchdy(67,j)             !!RDOC in (kg C)
        pdvar(52) = rchdy(68,j)             !!RDOC out (kg C)
        pdvar(53) = rchdy(69,j)             !!LDOC in (kg C)
        pdvar(54) = rchdy(70,j)             !!LDOC out (kg C)
        pdvar(55) = rchdy(71,j)             !!DIC in (kg C))
        pdvar(56) = rchdy(72,j)             !!DIC out (kg C))                
        pdvar(57) = rchdy(64,j) + rchdy(66,j) !!Total POC out (kg C)          !!62   SWAT-CUP
        pdvar(58) = rchdy(68,j) + rchdy(70,j) !!Total DOC out (kg C)          !!63   SWAT-CUP
        pdvar(59) = rchdy(64,j) + rchdy(66,j) + rchdy(68,j) + rchdy(70,j) !!Total DOC out (kg C)          !!63   SWAT-CUP
        !pdvar(59) = rchdy(73,j)+rchdy(74,j) !!Total DOC out (kg C)          !!63   SWAT-CUP
      
      
       !----finish assiging daily output values for carbon module------! 
!!  compute month and day given julian day
    ...
    ! &                                    (pdvr(ii), ii = 1, itotr)
	 ...
	 ! else
	    if (iscen == 1 .and. isproj == 0) then
           do kk=1,nstep_rch
             write (output_rch_num,5001) j, subgis(j), iida, kk, rch_dakm(j), (rchhr(ii,j,kk), ii = 1, 72)
	     end do
	     endif
	 ! endif
      else
...
!!    added for binary files 3/25/09 gsm line below and write (77777
  ...
  !  increase to 45 in loops below from 42 gsm 10/26/2011      
            if (iscen == 1 .and. isproj == 0) then
              ...
              if (icalen == 1) write (output_rch_num,5002) j, subgis(j), i_mo, icl(iida), iyr, rch_dakm(j),(pdvar(ii), ii = 1, 45)
    ...
    !!    added for binary files 3/25/09 gsm line below and write (77777
              if (ia_b == 1) then
                ...
                write (20,5000) j, subgis(j), iida, rch_dakm(j), (pdvar(ii), ii = 1, 45) 
            else if (iscen == 1 .and. isproj == 2) then
 ...
 6002 format ('REACH ',i5,1x,i8,1x,i2,1x,i2,1x,i4,1x,47e12.4,1x,i4) !!R666b 7/19/17 nbs
      end