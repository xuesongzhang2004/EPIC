...
!! calculate top width of channel at water level
      ...
      if (rchdep <= ch_d(jrch)) then
        ...
        rch_waterarea(jrch)=   topw*ch_l2(jrch)*1000   !!m2
      ! if(jrch==1) then
      ! print*,rch_waterarea(jrch), topw,ch_area(jrch),ch_w(2,jrch)
      ! end if 
!!	Time step of simulation (in hour)
      ...
      if (sdti > 0.) then
        !! calculate velocity and travel time
  	    vc = sdti / rcharea     !m^3/s  /m^2=m/s
        vel_chan(jrch) = vc
	  rttime = ch_l2(jrch) * 1000. / (3600. * vc)   !hr   

        !! calculate volume of water leaving reach on day
        scoef = 0.
 	    rtwtr = 0.
        !scoef = 2. * det / (2. * rttime + det)     !!R677 9/13/19 nbs
        scoef =  det / (rttime + det)
        if (scoef > 1.) scoef = 1.
        rtwtr = scoef * (wtrin + rchstor(jrch))
        !new storage coefficient replacement
        !rtwtr = vc * rcharea * 86400.              !!R677 9/13/19 nbs
        !rtwtr = amin1 (rtwtr, wtrin)               !!R677 9/13/19 nbs
