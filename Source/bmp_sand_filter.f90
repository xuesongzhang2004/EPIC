      subroutine bmp_sand_filter(kk,flw,sed)        !R668 1/30/2018 nbs
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: tsa,ffsa,vfiltr,mxvol,pdia,ksat,por,dp,dc,pden,alp,           &
      ...
      real, dimension(:) :: qpnd(0:nstep),qsw(0:nstep),qin(0:nstep),        &
...
!     &      qsw(ii),qloss
         ...
         if (qloss>0) then
            ...
            sed(3,ii) = sloss                               !!R666b 7/19/17 nbs                     
         end if
         ...
         If (qout(ii)<0.001)then
            ...
            sed(2,ii) = 0.                                  !!R666b 7/19/17 nbs
         Else
            if (sfsedstdev>0) then                          !!R666b 7/19/17 nbs
               ...
               ! water viscosity (g/cm-s) using 3rd order polynomial interpolation
                mu = -3.e-6 * tmpw ** 3 + 0.0006 * tmpw ** 2 -         &
                     0.0469 * tmpw + 1.7517                !!R666b 7/19/17 nbs       
	         mu = mu * 1.e-2
            ...
            End If
               ...
               effl = 1.5 * (dp / dc) ** 2              !!R666b 7/19/17 nbs 
              
               If (effl > 0.999) effl = 0.99
               If (effg > 0.999) effg = 0.99            
               If (effbr > 0.999) effbr = 0.99          !!R666b 7/19/17 nbs                     
           
               !contact efficiency
               effct = effl + effg + effbr              !!R666b 7/19/17 nbs 
               If (effct > 0.999) effct = 0.99
           
                 ! sediment removal efficiency
               sedrmeff = 1. - Exp(-1.5 * (1. - por) * alp * effct *   & !!R666b 7/19/17 nbs
...
!     & sed(2,ii)*1000,sloss*1000
      ...
      return
      end subroutine bmp_sand_filter            !R668 1/30/2018 nbs