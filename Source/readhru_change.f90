...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: eof
      real :: xm, sin_sl, epcohru, escohru,xx
      ...
      do
      read (hrufile_num,5100) titldum
      read (hrufile_num,*) hru_fr(ihru)
      read (hrufile_num,*) slsubbsn(ihru)
      read (hrufile_num,*) hru_slp(ihru) 
      read (hrufile_num,*) ov_n(ihru)
      read (hrufile_num,*) lat_ttime(ihru) 
      read (hrufile_num,*) lat_sed(ihru)   !read in in mg/L
      read (hrufile_num,*) slsoil(ihru)
      read (hrufile_num,*,iostat=eof) canmx(ihru) 
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) escohru
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) epcohru 
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) rsdin(ihru) 
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) erorgn(ihru) 
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) erorgp(ihru) 
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) pot_fr(ihru) 
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) fld_fr(ihru) 
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) rip_fr(ihru) 
      if (eof < 0) exit
      read (hrufile_num,5100,iostat=eof) titldum
      if (eof < 0) exit
!      if (ipot(ihru) == ihru) then   Srini pothole
        read (hrufile_num,*,iostat=eof) pot_tilemm(ihru)    !!NUBZ
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) pot_volxmm(ihru) 
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) pot_volmm(ihru) 
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) pot_nsed(ihru) 
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) pot_no3l(ihru)
        if (eof < 0) exit
!        read (hrufile_num,5100,iostat=eof) titldum
!        if (eof < 0) exit
!        read (hrufile_num,5100,iostat=eof) titldum
...
!        if (eof < 0) exit
!      end if
      read (hrufile_num,*,iostat=eof) dep_imp(ihru)
      if (eof < 0) exit
      read (hrufile_num,5100,iostat=eof) titldum       
      if (eof < 0) exit
      read (hrufile_num,5100,iostat=eof) titldum      
      ...
      read (hrufile_num,5100,iostat=eof) titldum           
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) evpot(ihru)
      if (eof < 0) exit
      read (hrufile_num,*,iostat=eof) dis_stream(ihru)
      if (eof < 0) exit
!! armen & stefan changes for SWAT-C
	read (hrufile_num,*,iostat=eof) cf(ihru)
	if (eof < 0) exit
	read (hrufile_num,*,iostat=eof) cfh(ihru)
	if (eof < 0) exit
	read (hrufile_num,*,iostat=eof) cfdec(ihru)
	if (eof < 0) exit
        ...
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) pot_k(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) n_reduc(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) n_lag(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) n_ln(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) n_lnco(ihru)
!-------------------------------------------------------Moriasi 4/8/2014        
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) surlag(ihru)
        if (eof < 0) exit
!-------------------------------------------------------Moriasi 4/8/2014 
        read (hrufile_num,*,iostat=eof) r2adj(ihru) !Soil retention parameter D. Moriasi 4/8/2014
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) cmn(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) cdn(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) nperco(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) phoskd(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) psp(ihru)
        if (eof < 0) exit 
        read (hrufile_num,*,iostat=eof) sdnco(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) iwetile(ihru)
        if (eof < 0) exit
        read (hrufile_num,*,iostat=eof) iwetgw(ihru)
	exit
      end do
