...
!!    Modified by N.Kannan, Blackland Research at Temple
      ...
      integer :: ii, j
      real :: inflow                                    !!R666 7/19/17 nbs
      if (inum1 <= 50 .and. inum1 > 0) then
      if (ievent_rch == 1 .and. inum2 == 1) then
        ...
        do ii = 1, nstep_rch
          ...
          if (hhvaroute(2,ihout,ii) > 0.001) then
            varii(1) = hhvaroute(2,ihout,ii) / (idt_rch * 60.)  !! urban modeling by J.Jeong 4/17/2008
            varii(2) = hhvaroute(3,ihout,ii) * 1.e6  / hhvaroute(2,ihout,ii)
!!       varii(3) = hhvaroute(4,ihout,ii) * 1000.                           !!R666 7/19/17 nbs                  
     ...
     !     !end if
		  ...
		  !write (50+inum1,2000) iyr, iida, ii-1, (varii(j), j = 1, 19)    !!R666 7/19/17 nbs
          write (50+inum1,2000) iyr, iida, ii-1, (varii(j), j = 1, 2)      !!R666 7/19/17 nbs
	    endif
        endif
        end do
      else
        !! Write daily values
        varii = 0.
        if (ievent_rch == 1) then         !! sum sub-daily to daily if needed
          ...
          if (varoute(2,ihout) > 0.1) then
            do ii = 1, nstep_rch
              do j = 3, mvaro
                varoute(j,ihout) = varoute(j,ihout) + hhvaroute(j,ihout,ii)
              end do
              varoute(1,ihout) = varoute(1,ihout) + hhvaroute(2,ihout,ii) * hhvaroute(1,ihout,ii)
            end do
      ...
      return
 2000 format (1x,i4,2x,i3,1x,i4,20(1x,e10.3))        !!R666 7/19/17 nbs
