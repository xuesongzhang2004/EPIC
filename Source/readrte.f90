...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      eof = 0
      do
      read (rtefile_num,5000) titldum
      read (rtefile_num,*) ch_w(2,irch)
      read (rtefile_num,*) ch_d(irch) 
      read (rtefile_num,*) ch_s(2,irch) 
      read (rtefile_num,*) ch_l2(irch)
      read (rtefile_num,*) ch_n(2,irch) 
      read (rtefile_num,*) ch_k(2,irch) 
      read (rtefile_num,*) ch_cov1(irch) 
      read (rtefile_num,*,iostat=eof) ch_cov2(irch) 
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_wdr(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) alpha_bnk(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) icanal(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_onco(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_opco(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) chside(irch)
	  if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_bnk_bd(irch)
        if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_bed_bd(irch)
	  if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_bnk_kd(irch)
	  if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_bed_kd(irch)
	  if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_bnk_d50(irch)
	  if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) ch_bed_d50(irch)
	  if (eof < 0) exit
	  read (rtefile_num,5000,iostat=eof) tc_bnk(irch)
	  if (eof < 0) exit
	  read (rtefile_num,5000,iostat=eof) tc_bed(irch)
	  if (eof < 0) exit
      read (rtefile_num,5100,iostat=eof) (ch_erodmo(irch,mo), mo = 1,12)
	  if (eof < 0) exit
	  read (rtefile_num,*,iostat=eof) ch_eqn(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) prf(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) spcon(irch)
      if (eof < 0) exit
      read (rtefile_num,*,iostat=eof) spexp(irch)
      if (eof < 0) exit
