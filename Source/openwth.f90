...
!! open precip files and read elevation
     ...
     !!     open (100+j,file=rfile(j),recl=800)
          open (rfile_num+j,file=rfile(j),recl=1850)
          read (rfile_num+j,5000) titldum
          read (rfile_num+j,5000) titldum
          read (rfile_num+j,5000) titldum
          if (ievent == 0) then   !daily records
            read (rfile_num+j,5001) (elevp(k), k = kk1, kk2)          !5001 format (7x,1800i5)
          else                   !subdaily records          
            read (rfile_num+j,5003) (elevp(k), k = kk1, kk2)          !5003 format (12x,1800i5)
          endif
        end if
      end do
  ...
  !!        open (118+j,file=tfile(j),recl=800)
          open (tfile_num+j,file=tfile(j),recl=20000)
          read (tfile_num+j,5000) titldum                 ! 5000 format (a80)
          read (tfile_num+j,5000) titldum
          read (tfile_num+j,5000) titldum
          read (tfile_num+j,5002) (elevt(k), k = kk1, kk2)     !5002 format (7x,1800i10)
        end if
      end do

        if (slrfile /= '             ') then
   !!       open (137,file=slrfile,recl=800)
          open (slrfile_num,file=slrfile,recl=16000)        !!R681 5/26/20 nbs
          read (slrfile_num,5000) titldum
        end if

        if (rhfile /= '             ') then
  !!        open (138,file=rhfile,recl=800)
          open (rhfile_num,file=rhfile,recl=16000)          !!R681 5/26/20 nbs    
          read (rhfile_num,5000) titldum
        end if

        if (wndfile /= '             ') then
   !!       open (139,file=wndfile,recl=800)
          open (wndfile_num,file=wndfile,recl=16000)        !!R681 5/26/20 nbs
          read (wndfile_num,5000) titldum
        end if

        if (petfile /= '             ') then
          open (petfile_num,file=petfile)
          read (petfile_num,5000) titldum
        end if
