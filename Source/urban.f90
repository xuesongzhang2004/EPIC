...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: cod, sus_sol, tn, tp, urbk, turo, dirto, durf, rp1, dirt, xx, tno3
      integer :: j
      real, external :: Regres, sweep
      real :: exp
      ...
      select case (iurban(j))
        ...
        if (precipday > .1 .and. surfq(j) > .1) then
          ...
          sedyld(j) = (.001 * sus_sol) * fimp(urblu(j)) + sedyld(j) * (1. - fimp(urblu(j)))

          !! The sediment loading from urban imprevious area is assumed 
	    !! to be all sitly particles
          silyld(j) = (.001 * sus_sol) * fimp(urblu(j)) + silyld(j) * (1. - fimp(urblu(j)))
          ...
          sedorgn(j) = (.7 * tn / (hru_km(j) * 100.)) * fimp(urblu(j)) + sedorgn(j) * (1. - fimp(urblu(j)))
          surqno3(j) = (.3 * tn / (hru_km(j) * 100.)) * fimp(urblu(j)) + surqno3(j) * (1. - fimp(urblu(j)))
          sedorgp(j) = (.75 * tp / (hru_km(j) * 100.)) * fimp(urblu(j)) +  sedorgp(j) * (1. - fimp(urblu(j)))
          surqsolp(j) = .25 * tp / (hru_km(j) * 100.) * fimp(urblu(j)) + surqsolp(j) * (1. - fimp(urblu(j)))
        endif
        ...
        if (surfq(j) > 0.1) then
          ...
          sedyld(j) = (.001 * sus_sol * hru_ha(j)) * fimp(urblu(j)) + sedyld(j) * (1. - fimp(urblu(j)))

          !! The sediment loading from urban imprevious area is assumed 
	    !! to be all sitly particles
          silyld(j) = (.001 * sus_sol * hru_ha(j)) * fimp(urblu(j)) + silyld(j) * (1. - fimp(urblu(j)))
          sanyld(j) = sanyld(j) * (1. - fimp(urblu(j)))
          clayld(j) = clayld(j) * (1. - fimp(urblu(j)))
          sagyld(j) = sagyld(j) * (1. - fimp(urblu(j)))
          lagyld(j) = lagyld(j) * (1. - fimp(urblu(j)))

          surqno3(j) = tno3 * fimp(urblu(j)) + surqno3(j) * (1. - fimp(urblu(j)))
          sedorgn(j) = (tn - tno3) * fimp(urblu(j)) + sedorgn(j) * (1. - fimp(urblu(j)))
          sedorgp(j) = .75 * tp * fimp(urblu(j)) + sedorgp(j) *  (1. - fimp(urblu(j)))
          surqsolp(j) = .25 * tp * fimp(urblu(j)) + surqsolp(j) *  (1. - fimp(urblu(j)))
        else
          !! dry day
          twash(j) = twash(j) + 1.
