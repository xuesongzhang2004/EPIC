...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use carbon_para !added by Du, for carbon module 
      use parm 
      use parm_control
      use parm_output
      use parm_subC
      ...
      integer :: j, imho    ! added by Du, for carbon module   
      allocate (alph_e(mhru))
      allocate (co_p(mhru))

      imho = max(mhru,20)
      allocate (ipdhru(imho))           !!ipdhru(:)   |none        |HRUs whose output information will be printed to the output.hru and output.wtr files
      !read (file_cio2_num,*) (ipdhru(ii),ii=1,20)
...
!! initialize variables    
      ...
      allocate (hhvaroute(mvaro,mhyd,nstep_rch))  !! from 24 to nstep for urban           
      
      mvaro2 = 96 !!--------output2.rch---------res---------------------------------
      !!---------subdaily-----output2,3,4,5,6,7.rch--------------------
      allocate (rchhr2(mrcho2,mch,nstep_rch))
      allocate (hhvaroute2(mvaro2,mhyd,nstep_rch))   
      !allocate_parms_output-Qi.f90(6):       allocate (varoute2(mvaro2,mhyd))
      
      mhruo = 153 !!145  !!---------output.hru---------------------------------------
      allocate (heds(mhruo))
      allocate (icols(mhruo))
      allocate (ipdvas(mhruo))
      allocate (hrumono(mhruo,mhru))               
      allocate (hruyro(mhruo,mhru))                
      allocate (hruaao(mhruo,mhru))                


      mhruo1 = 138   !!--------output.hru-----------------------------------------
      allocate (hrumonN(mhruo1,mhru))                 
      allocate (hruyrN(mhruo1,mhru))                  
      allocate (hruaaN(mhruo1,mhru))                  
...
!!  increased ipdvar(42) to 45 to add Total N/Total P/NO3conc(mg/l)
      allocate (ipdvar(mrcho))       !!-----------------------output.rch-----------------------------------------------------
      
      mrcho1 = 76     !!--------output.rch------------------------------------------
      allocate (rchaao(mrcho1,mxsubch))       
      allocate (rchdy(mrcho1,mxsubch))        
      allocate (rchmono(mrcho1,mxsubch))      
      allocate (rchyro(mrcho1,mxsubch))       

      mrcho2 = 96    !! --------output2.rch------------------------------------------
      allocate (rchaao2(mrcho2,mxsubch))    
      allocate (rchyro2(mrcho2,mxsubch))    
      allocate (rchmono2(mrcho2,mxsubch))   
      allocate (rchdy2(mrcho2,mxsubch))     
...
!!    arrays which contain data related to subbasin output
      ...
      allocate (wshdaaN(mstdo))
      allocate (wshddayN(mstdo))
      allocate (wshdmonN(mstdo))
      allocate (wshdyrN(mstdo))


      motot = 1200            !! (100 years limit)   !!R683 1/13/22 nbs
!!!   atmospheric deposition by month
      ...
      allocate (hedse(9))
      allocate (hedsp(29))
      !hedsc(:),hedsn(:),hedse(:),hedsp(:)  !hedsc(47),hedsn(53),hedse(9),hedsp(29)
      !hedsc        output_C.hru
      !hedsn        output_N.hru
      !hedse        output_E.hru
      !hedsp        output_P.hru
      ...
      allocate (tilep(mhru))                        !!R683 1/13/22 nbs
      allocate (cdn(mhru))
      allocate (cmn(mhru))
...
!      allocate (ch_erod(mch))
      ...
      allocate (rch_SolP(mch))
      allocate (drift(mch))
      allocate (flwin(mch))
      allocate (flwout(mch))
      allocate (icanal(mch))
      allocate (rch_NO3(mch))
      allocate (rch_NO2(mch))
      allocate (rch_OrgN(mch))
      allocate (rch_OrgP(mch))
...
!!    arrays which contain data related to subbasins
      allocate (sub_tilep(msub))                    !!R683 1/13/22 nbs
...
!      allocate (hqd(nstep*3+1))  !! was 73, changed for urban
  ...
  !!    allocate (uh(msub,49))
      ...
      allocate (sub_hhqd(msub,nstep_rch))  ! 24 changed to nstep 4 urban modeling  Oct. 19,2007   
      allocate (sub_hhwtmp(msub,nstep_rch))   ! 24 changed to nstep 4 urban modeling  Oct. 19,2007   
...
!  added 1/27/09 when making septic changes
      ...
      allocate (sol_nh4(mlyr,mhru))      
      allocate (sol_nh3(mlyr,mhru))     
      allocate (sol_no3(mlyr,mhru))    
      allocate (sol_urea(mlyr,mhru))    
      ...
      allocate (sol_tmp(mlyr+10,mhru))    !!  Physically-based soil temperature
...
!!    arrays which contain data related to HRUs
      ...
      allocate (cnday(mhru))
!    Drainmod tile equations  01/2006 
      allocate (cont_cn(21,mhru))         !!R682 10/20/21 nbs
	  allocate (cumei(mhru))
	  allocate (cumeira(mhru))
	  allocate (cumrt(mhru))
	  allocate (cumrai(mhru))
!    Drainmod tile equations  01/2006
      allocate (cont_p(21,mhru))                !!R682 10/20/21 nbs
      allocate (cropno_upd(21,mhru))            !!R682 10/20/21 nbs
...
!    Drainmod tile equations  01/2006
      ...
      allocate (doxq(mhru))
      allocate (drain_d(21,mhru))               !!R682 10/20/21 nbs
      allocate (drain_idep(21,mhru))            !!R682 10/20/21 nbs
      allocate (drain_t(21,mhru))               !!R682 10/20/21 nbs
      allocate (drain_g(21,mhru))               !!R682 10/20/21 nbs
      ...
      allocate (filt_w(21,mhru))               !!R682 10/20/21 nbs 
      allocate (fire_cn(21,mhru))              !!R682 10/20/21 nbs            
      ...
      allocate (gwati(21,mhru))                 !!R682 10/20/21 nbs
      allocate (gwatn(21,mhru))                 !!R682 10/20/21 nbs
      allocate (gwatl(21,mhru))                 !!R682 10/20/21 nbs
      allocate (gwatw(21,mhru))                 !!R682 10/20/21 nbs
      allocate (gwatd(21,mhru))                 !!R682 10/20/21 nbs
      allocate (gwatveg(21,mhru))               !!R682 10/20/21 nbs
      allocate (gwats(21,mhru))                 !!R682 10/20/21 nbs
      allocate (gwatspcon(21,mhru))             !!R682 10/20/21 nbs
      allocate (gwata(21,mhru))                 !!R682 10/20/21 nbs
      ...
      allocate (gwno3(mhru))
      allocate (gwqmn(mhru))
      allocate (hi_upd(21,mhru))                !!R682 10/20/21 nbs
...
!!      allocate (ipot(mhru))
      ...
      allocate (laimx_upd(21,mhru))                 !!R682 10/20/21 nbs
...
!    Drainmod tile equations  01/2006 
      ...
      allocate (yield(mhru))
      allocate (yieldgrn(mhru))
      allocate (yieldbms(mhru))
      allocate (yieldtbr(mhru))
      allocate (yieldn(mhru))
      allocate (yieldp(mhru))
      allocate (yieldrsd(mhru))
      ...
      allocate (twlpnd(mhru))     !!srini pond/wet infiltration to shallow gw storage  !!R669 4/20/18 nbs
      allocate (twlwet(mhru))     !!srini pond/wet infiltration to shallow gw storage  !!R669 4/20/18 nbs
...
!    Drainmod tile equations  01/2006 
      ...
      allocate (strip_n(21,mhru))                   !!R682 10/20/21 nbs
!!    Drainmod tile equations  01/2006 
	allocate (strip_cn(21,mhru))                    !!R682 10/20/21 nbs
	allocate (strip_c(21,mhru))                     !!R682 10/20/21 nbs
	allocate (strip_p(21,mhru))                     !!R682 10/20/21 nbs
	allocate (stmaxd(mhru))
      ...
      allocate (terr_cn(21,mhru))                   !!R682 10/20/21 nbs
      allocate (terr_p(21,mhru))                    !!R682 10/20/21 nbs
      allocate (terr_sl(21,mhru))                   !!R682 10/20/21 nbs
      ...
      allocate (frad(mhru,nstep1))     
!      allocate (hhsubp(mhru,24))

 !     allocate (rhrbsb(24))
      allocate (rstpbsb(nstep))
      allocate (rainsub(mhru,nstep1))   
      allocate (precipdt(nstep+1))

      allocate (bss(4+4,mhru))   ! modified by Du for carbon lag calculation   !! lateral flow water temperature
      ...
      allocate (surf_bs(17+9,mhru))  ! modified by Du for carbon lag calculation   HRU water temperature
...
!!    arrays
      ...
      allocate (rch_hAlg(nstep_rch))
      allocate (hbactlp(nstep_rch))
      allocate (hbactp(nstep_rch))
      allocate (rch_hCBOD(nstep_rch))
      allocate (rch_hChla(nstep_rch))
      allocate (hdepth(nstep_rch))      ! changed as per nstep_rch  !nstep_rch Mar 19,2008
      ! allocate (hdepth_end(nstep_rch))
      allocate (rch_hDOX(nstep_rch))
      allocate (hharea(nstep_rch))	  ! changed as per nstep_rch  !nstep_rch Mar 19,2008
      ! allocate (hharea_end(nstep_rch))	  ! changed as per nstep_rch  !nstep_rch Mar 19,2008
      !allocate (ch_warea_end(nstep_rch))
      allocate (hhqday(nstep_rch))  ! changed as  nstep_rch  Oct. 18, 2007
      allocate (hhstor(nstep_rch))  ! changed as per nstep_rch   !nstep_rch Mar 19,2008
      allocate (hhtime(nstep_rch))  ! changed as per nstep_rch   !nstep_rch Mar 19,2008
      allocate (rch_hNH4(nstep_rch))
      allocate (rch_hNO2(nstep_rch))
      allocate (rch_hNO3(nstep_rch))
      allocate (rch_hOrgN(nstep_rch))
      allocate (rch_hOrgP(nstep_rch))
      allocate (hrchwtr(nstep_rch))  ! changed as per nstep_rch  !nstep_rch Mar 19,2008
      allocate (hrtwtr(nstep_rch))   ! changed as per nstep_rch  !nstep_rch Mar 19,2008
      allocate (hsdti(nstep_rch))	   ! changed as per nstep_rch  !nstep_rch Mar 19,2008
      allocate (hsedst(nstep_rch))
      allocate (hsedyld(nstep_rch))
      allocate (rch_hSolP(nstep_rch))
      allocate (hsolpst(nstep_rch))
      allocate (hsorpst(nstep_rch))
...
!! Arrays for subdaily erosion modeling by Jaehak Jeong
	allocate (hhsedy(mhru,nstep),rhy(nstep_rch),ovrlnd_dt(mhru,nstep))                   
      allocate (snam(mhru),hydgrp(mhru),kirr(mhru))
	allocate (dratio(msub),init_abstrc(mhru))
	allocate (sub_subp_dt(msub,nstep1),sub_hhsedy(msub,nstep_rch))                
	allocate (sub_atmp(msub,nstep_rch),bmp_recharge(msub))                         
	allocate (rchhr(mrcho1,mch,nstep_rch), hrttlc(nstep_rch))                         
	allocate (hrtevp(nstep_rch))                                                   
	allocate (hhresflwi(nstep_rch), hhresflwo(nstep_rch),hhressedi(nstep_rch), hhressedo(nstep_rch)) 
...
!! Filter Strip variable allocation MJW
      ...
      allocate (vfsch(mhru))
      allocate (vfsi(mhru))
      allocate (filter_i(21,mhru))                      !!R682 10/20/21 nbs
      allocate (filter_ratio(21,mhru))                  !!R682 10/20/21 nbs
      allocate (filter_con(21,mhru))                    !!R682 10/20/21 nbs
      allocate (filter_ch(21,mhru))                     !!R682 10/20/21 nbs
...
!! Variables for soil P and additional operations mjw
	...
	allocate (ro_bmp_bact(21,mhru))                 !!R682 10/20/21 nbs
...
!!    van Genuchten equation's coefficients
      !allocate(lid_vgcl,lid_vgcm,lid_qsurf_total, &            !!R666 6/19/17 nbs
      !	lid_farea_sum)
...
!!    LID general
      ...
      allocate(mgtyr_hru(mhru))
           
       !FLOOD ROUTING                                   !!R666 7/19/17 nbs
       allocate(QHY(nstep+1,mhyd,4), NHY(4*msub))       !!R666 7/19/17 nbs
       allocate(RCHX(msub),RCSS(msub),QCAP(msub),CHXA(msub),CHXP(msub))   !!R666 7/19/17 nbs
