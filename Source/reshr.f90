...
!! calculate water balance for day
        resev = 6. * pet_day * ressa / nstep_rch		!! urban modeling by J.Jeong
        ressep = res_k(jres) * ressa * 240./ nstep_rch	!! urban modeling by J.Jeong
...
!! if reservoir volume is greater than zero
          ...
          select case (iresco(jres))
            case (0)                    !! uncontrolled reservoir
              ...
              if (res_vol(jres) > res_pvol(jres)) then
                ...
                if (res_rr(jres) > vvr) then
                  hhresflwo(k) = (hhresflwo(k) + vvr) / nstep_rch !m3
                else
               hhresflwo(k) = (hhresflwo(k) + res_rr(jres)) / nstep_rch !m3
                endif
              endif

            case (1)                   !! use measured monthly outflow
            hhresflwo(k) = res_out(jres,i_mo,curyr) * 86400. / nstep_rch !m3/s -> m3
            ...
            case (3)                   !! use measured daily outflow
              if (k==1) read (350+jres,5000) flw
  		      hhresflwo(k) = 86400. * flw / nstep_rch		!! m3, urban modeling by J.Jeong

          end select

        !! check calculated outflow against specified max and min values
          if (hhresflwo(k)<oflowmn(i_mo,jres)*86400./nstep_rch) then
            resflwo = oflowmn(i_mo,jres) * 86400. / nstep_rch
          end if
          if (hhresflwo(k)>oflowmx(i_mo,jres)*86400./nstep_rch.and. &
	  			oflowmx(i_mo,jres)>0.) then
             hhresflwo(k) = oflowmx(i_mo,jres) * 86400. / nstep_rch
          endif
        ...
        !! subtract consumptive water use from reservoir storage
          xx = 0.
          xx = wuresn(i_mo,jres) / nstep_rch		!! urban modeling by J.Jeong
