...
!!  Code developed by J. Jeong and N. Kannan, BRC.
	...
	real :: ke_direct, ke_leaf, ke_total,ph_eff, c
...
!! Soil detachability values adopted from EUROSEM User Guide (Table 1)
	  if ((percent_clay>=40.) .and. (percent_sand>=20.) .and. (percent_sand<=45.)) then
	    erod_k = 2.0 !clay
      elseif ((percent_clay>=27.) .and. (percent_sand>=20.) .and. (percent_sand<=45.)) then
      ...
      elseif ((percent_clay>=7.) .and. (percent_sand<=52.) .and. (percent_silt>=28.)) then
	    erod_k = 2.0 !Loam
...
!!	canopy cover is assumed to be 100% if LAI>=1
      ...
      do k=1,nstep
	      ...
	      !! Rainfall kinetic energy generated by leaf drainage (J/m^2)
	        ph_eff = 0.5 * cht(j)                    !ph_eff     	|m			   |effective plant height
	        ke_leaf = 15.8 * ph_eff ** 0.5 - 5.87
	    ...
	    !! Impervious area of HRU
	      if(urblu(j)>0) sedspl = sedspl * (1.- fimp(urblu(j)))     !sedspl		|tons		   |sediment yield by rainfall impact during time step
	    ...
	    !! specific weight of water at 5 centigrate =9807N/m3
	      ...
	      sedov = 11.02 * rill_mult * usle_k(j) * c_factor * c * bed_shear ** eros_expo ! kg/hour/m2
	      if(ievent > 0) then
