...
!!    solc_orgn     |kg N/ha       |total organic N in HRU soil profile
...
!! calculate average annual values for reach data
      rchaao = rchaao / yrs
      ! if(cswat == 2) rchaao2 = rchaao2 / yrs                
...
!! calculate average annual values for watershed data
      ...
      do j = 1, nhru
        ...
        basno3f = basno3f + solc_no3(j) * hru_dafr(j)
        basorgnf = basorgnf + solc_orgn(j) * hru_dafr(j)
        basminpf = basminpf + summinp * hru_dafr(j)
        basorgpf = basorgpf + solc_orgp(j) * hru_dafr(j)
      end do
...
!! write average annual pesticide data (output.pes)
      if (iprp == 1) then
        write (output_pst_num,5500)
        do j = 1, nhru
          if (hrupest(j) == 1) then
                write (output_pst_num,5600) subnum(j), hruno(j), yrs,               &
...
!! write to hydrograph output file
      do idmm = 1, mhyd
        ic = ihouts(idmm)
        if (ic > 0) then
        write(hyd_out_num,9400) icodes(idmm), ic, inum1s(idmm), inum2s(idmm), &
...
!! write life span of septic HRUs (time to first failure, years)
    write(septic_out_num,'(1/,a50)') ' Year the first failure occurred'  !!R671 4/5/19 nbs
      write(septic_out_num,'(1/,20x,a5,a7)') 'HRU',' Failyr'             !!R671 4/5/19 nbs
      do j=1,nhru
	  if (isep_opt(j)/=0) then
        if(failyr(j)==0) write(173,'(20x,i5,a8)') j,' No Fail'          !!R671 4/5/19 nbs
        if(failyr(j)/=0) write(173,'(20x,i5,f7.3)') j,failyr(j)         !!R671 4/5/19 nbs
        end if
      end do
