...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: jres, jj
      real :: vol, sed, vvr, targ, xx, flw, x1, res_h, res_h1, res_qi, velofl, trapres, susp
...
!! if reservoir volume is greater than zero
 ...
 !               xx = Min(sub_sw(res_sub(jres))/sub_sumfc(res_sub(jres)),   !!R682 10/20/21 nbs
 !    &                                                               1.)   !!R682 10/20/21 nbs
 !               targ = res_pvol(jres) + .5 * (1. - xx) *                   !!R682 10/20/21 nbs             
 !    &                                 (res_evol(jres) - res_pvol(jres))   !!R682 10/20/21 nbs
                targ = res_pvol(jres)                                       !!R682 10/20/21 nbs              
                end if
              else
 ...
 !    &                                 (res_evol(jres) - res_pvol(jres))       !!R682 10/20/21 nbs
          ...
          case (3)                   !! use measured daily outflow
            flw = 0.
            read (resdayo_num+jres,5000) flw
            resflwo = 86400. * flw
