...
!! no elevation bands
	...
	ib = 1
        ...
        snotmp(j)=snotmp(j) * (1. - sub_timp(ib,isub)) + tmpav(j) * sub_timp(ib,isub)
      ...
      !!---------------------------------------TSWAT---------------------------
	   ...
	   a=1
	   b=0.3
	   !!sno_den(j)=(a+ sqrt(a**2 +b*60.1*sno_hru(j))) /2/1000  !! g/cm3
       !!if (snomlt > 5.) sno_den(j) = 1
       sno_den(j)=(b+a*sno_hru(j)/1000)!! g/cm^2
	   ...
	   sno_dep(j)=sno_hru(j)/sno_den(j) !!mm
	            !! g/cm3
	   if (sno_hru(j)<=0) sno_den(j)=0
        !!if(snomlt>1) k_sno(j)=864*2.1
