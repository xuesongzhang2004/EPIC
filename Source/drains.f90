...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real:: k2, k3, k4, k5, k6 
      real :: y1, above, nlayer, x, sum, deep, xx, hdmin, storro
      real :: stor, dflux, em, ddranp, dot
      real:: abs
...
!     & 	  log(e)) - 1.15))
      ...
      if (potsa(j) <= 0.) then ! determine stor
        ...
        stor = pot_vol(j)!/(potsa(j)*1000) pot_vol is already in mm, Jaehak 2020  !!R682 10/20/21 nbs
      endif
