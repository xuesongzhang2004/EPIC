...
!! algal growth
	   ...
	   If (lambda > lambda0) lambda = lambda0
         ...
         rch_Alg(jrch) = 0.
         rch_Alg(jrch) = algcon + (Theta(gra,thgra,wtmp) * algcon -       &
	...
	!! JGA added to set algae limit *****
	   if (rch_Alg(jrch) > 5000.) rch_Alg(jrch) = 5000.
         if (rch_Alg(jrch) > dcoef * algcon) then
          rch_Alg(jrch) = dcoef * algcon
        end if
...
!        zz = zz + red_fac
         rch_OrgN(jrch) = 0.
         rch_OrgN(jrch) = orgncon + (xx - yy - zz) * tday
        ...
        rch_NH4(jrch) = 0.
        rch_NH4(jrch) = nh3con + (ww - xx + yy - zz) * tday
        ...
        rch_NO2(jrch) = 0.
        rch_NO2(jrch) = no2con + (yy - zz) * tday
	  ...
	  if (rch_NO2(jrch) > dcoef * no2con .and. no2con > 0.)  rch_NO2(jrch) = dcoef * no2con
        ...
        rch_NO3(jrch) = 0.
        rch_NO3(jrch) = no3con + (yy - zz) * tday
...
!! phosphorus calculations
        ...
        rch_OrgP(jrch) = 0.
        rch_OrgP(jrch) = orgpcon + (xx - yy - zz) * tday
        ...
        rch_SolP(jrch) = 0.
        rch_SolP(jrch) = solpcon + (xx + yy - zz) * tday
