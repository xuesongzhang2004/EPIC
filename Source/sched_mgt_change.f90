...
1002  format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,10x,f10.2,70x,2i7)
          ...
          case (3)   !! fertilizer operation
            ...
            frt_surface = mgt5op(nop(j),j)  !|fraction of fertilizer which is applied to the top 10 mm of soil (the remaining fraction is applied to first soil layer)
...
1004  format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,20x,f10.2,10x,5f10.2)
          ...
          case (5)   !! harvest and kill operation
            ...
            if (imgt ==1) then
             ...
             sol_rsd(1,j), sol_sumno3(j),sol_sumsolp(j),yield(j),          &
...
1003  format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,30x,f10.2)
          ...
          case (7)  !! harvest only operation
            hi_bms = mgt5op(nop(j),j)     !! biomass (1) or grain (0) harvest code
            ...
            if (imgt == 1) then
             ...
             sol_rsd(1,j), sol_sumno3(j), sol_sumsolp(j), yield(j),            &
             strsn_sum(j), strsp_sum(j), strstmp_sum(j), strsw_sum(j),      &
             strsa_sum(j), yieldgrn(j), yieldbms(j), yieldtbr(j), yieldrsd(j),          &
             yieldn(j), yieldp(j)
            end if
