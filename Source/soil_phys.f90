...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: nly, j
      real :: xx, sumpor, dg, pormm, rock_factor, drpor, sa, cl, si
...
!!    calculate composite usle value
      ...
      rock_factor = Exp(-.053 * sol_rock(1,i))
      !usle_mult(i) = sol_rock(1,i) * usle_k(i) * usle_p(i) * usle_ls(i) * 11.8
      usle_mult(i) = rock_factor * usle_k(i) * usle_p(i) * usle_ls(i) * 11.8
...
!!    initialize water/drainage coefs for each soil layer
      ...
      do j = 1, nly
        ...
        crdep(j,i) = sol_crk(i) * 0.916 * Exp(-.0012 * sol_z(j,i)) * dg                 !!crdep(:,:)    |mm            |maximum or potential crack volume
                                                                                        !!dg = sol_z(j,i) - xx (thickness in mm)
...
!!    initialize watershed water parameters
      ...
      call curno(cn2(i),i) !! J.Jeong 4/18/2008             !readmgt-Qi.f90(324):      read (mgtfile_num,*) cn2(ihru)
