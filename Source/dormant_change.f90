...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: X1, X8, X10, XX, YY, ZZ, XZ, YZ, RLN, RLR, orgc_f
      orgc_f = 0.
      BLG1 = 0.
      BLG2 = 0.
      BLG3 = 0.
      ...
      j = ihru
        ...
        if (iyr == 2014 .and. i == 361 .and. j == 1)  then
            print *, iyr, i, j, "STOP"
        end if
        ...
        select case (idc(idplt(j)))
          ...
          case (7)
            ...
            if (cswat == 2) then
                ...
                rsdn_d(j) = rsdn_d(j) + resnew * pltfr_n(j)
            end if
            !!add by zhang
            !!===================
          ...
          OrgC_Plt2Rsd(j)=OrgC_Plt2Rsd(j)+ resnew * CFB        !!residue C input
          OrgN_Plt2Rsd(j)=OrgN_Plt2Rsd(j)+ resnew * pltfr_n(j)   !!residue N input 
          OrgP_Plt2Rsd(j)=OrgP_Plt2Rsd(j)+ resnew * pltfr_p(j)   !!residue P input 
            ...
            if (cswat == 2 .and. resnew > 0.) then
	          ...
	          if (resnew > 10.) then   
        	    ...
        	    RLN = (resnew * CLG/(resnew_n+1.E-5))                   !!ratio of litter to litter N
	          ...
	          !sol_LSLC(1,j) = sol_LSLC(1,j) + RLR*CFB*LSF * resnew
              ...
              if (resnew > 10.) then              
                ...
                sol_no3(1,j) = sol_no3(1,j) * (1-abs_f(j))
                sol_nh4(1,j) = sol_nh4(1,j) * (1-abs_f(j))
              end if
          ...
          OrgP_Plt2Rsd(j)=OrgP_Plt2Rsd(j)+ resnew * pltfr_p(j)   !!residue P input
            ...
            end if  !if (cswat == 2 .and. resnew > 0.) then
            !!insert new biomss by zhang
            !!===========================
