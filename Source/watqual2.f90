...
!      real :: thrk5 = 1.047, thrk6 = 1.0, thrs6 = 1.024, thrs7 = 1.0
	real  ::   dalgae, dchla, dorgn, dnh4, dno2, dno3,dorgp,dsolp
      real  ::   dbod, ddisox, o2con2, setl, hh, o2proc
      real, external :: theta
...
!! concentrations
         ...
         algcon = rch_Alg(jrch)
         orgncon = rch_OrgN(jrch)
         nh3con = rch_NH4(jrch)
         no2con = rch_NO2(jrch)
         no3con = rch_NO3(jrch)
         orgpcon =  rch_OrgP(jrch)
         solpcon = rch_SolP(jrch)
...
!! algal growth
         ...
         dalgae = algcon + (Theta(gra,thgra,wtmp) * algcon - Theta(rhoq,thrho,wtmp) * algcon - setl * algcon) * tday
         if (dalgae < 0.00001) rch_Alg(jrch) = 0.00001
...
!! end phosphorus calculations
         ...
         rch_Alg(jrch) = (algin * wtrin + dalgae * rchwtr) / wtrtot

         rch_OrgN(jrch) = (orgnin * wtrin + dorgn * rchwtr) / wtrtot
         rch_NH4(jrch) = (ammoin * wtrin +  dnh4 * rchwtr) / wtrtot
         rch_NO2(jrch) = (nitritin * wtrin + dno2  * rchwtr) / wtrtot
         rch_NO3(jrch) = (nitratin * wtrin + dno3  * rchwtr) / wtrtot
         rch_OrgP(jrch) = (orgpin * wtrin +  dorgp * rchwtr) / wtrtot
         rch_SolP(jrch) = (dispin * wtrin +  dsolp * rchwtr) / wtrtot
