...
!! elevation bands
        ...
        do ib = 1, 10
          !ratio = precipday / subp(j)            !!R664 02/15/17  nbs  then R666 7/19/17 nbs
          !pcpband(ib,j) = ratio * pcpband(ib,j)  !!R664 02/15/17  nbs  then R666 7/19/17 nbs
          if (subp(j) > 0.) then                        !!R666 7/19/17 nbs
            ratio = precipday / subp(j)                 !!R666 7/19/17 nbs
            pcpband(ib,j) = ratio * pcpband(ib,j)       !!R666 7/19/17 nbs
          else                                          !!R666 7/19/17 nbs
            pcpband(ib,j) = 0.                          !!R666 7/19/17 nbs
          end if                                        !!R666 7/19/17 nbs
          if (elevb_fr(ib,isub) < 0.) exit
          snotmpeb(ib,j) = snotmpeb(ib,j) * (1.-sub_timp(ib,isub)) + tavband(ib,j) * sub_timp(ib,isub)
          ...
          if (tavband(ib,j) < sub_sftmp(ib,isub)) then
            ...
            if (tmxband(ib,j) > sub_smtmp(ib,isub)) then
              smfac = 0.
              smleb = 0.
              smfac = (sub_smfmx(ib,isub) + sub_smfmn(ib,isub)) / 2. +  Sin((iida - 81) / 58.09) *                                       &
                    (sub_smfmx(ib,isub) - sub_smfmn(ib,isub)) / 2.    !! 365/2pi = 58.09
              smleb = smfac * (((snotmpeb(ib,j) + tmxband(ib,j)) / 2.) - sub_smtmp(ib,isub))
