...
!!--------------------Du-----------water temperature------------------------------------------------------
       integer psub,pday
       if(iwtmp_sub==1) then        !iwtmp_sub = 0 !! 0=default ; 1= F's subbasin water temperaute; 2= Qi's hru/subbasin water temperature WHEN tswat=1
      !moving a day down
       do psub=1,msub
        do pday=30,2,-1
            tmpav_lag(pday,psub)=tmpav_lag(pday-1,psub)
        end do
       enddo
      !fill yesterday's average temperature
       do psub=1,msub
       ! tmpav_lag(1,psub)=tmpav(hru1(psub))
         tmpav_lag(1,psub)=tmpav_pre(hru1(psub))   !!!hru1 is the first HRU in a subbasin
         !allocate (tmpav_pre(itempa)); itempa = Max(mhru,mch)
...
!! Potential ET: Read in data !!
      if (ipet == 3) then
        if (ifirstpet == 0) then
          read (petfile_num,5100) petmeas
        else
          ifirstpet = 0
          do 
          iyp = 0
          idap = 0
          read (petfile_num,5000) iyp, idap, petmeas
...
!! Precipitation and Temperature if = 2
      inum3sprev = 0    !|subbasin number of previous HRU
      do k = 1, nhru
        !! use same generated data for all HRUs in a subbasin
        if (hru_sub(k) == inum3sprev .and. hru_sub(k) /= 0) then
          if (tmpsim == 2) then
            tmx(k) = tmxbsb                 !|deg C         |generated maximum temperature for subbasin
            tmn(k) = tmnbsb
          end if
          if (pcpsim == 2) then
            subp(k) = rbsb
          ...
          if (slrsim == 2) then
            ...
            hru_rmx(k) = rmxbsb
            dayl(k) = daylbsb
            npcp(k) = npcpbsb       !!    npcp(:)     |none          |prior day category |1 dry day|2 wet day
            do ii = 1, nstep
              frad(k,ii) = fradbsb(ii)       !|none          |hourly solar radiation fractions for subbasin
            end do
          end if
...
!! Climate Change Adjustments !!
      do k = 1, nhru
        ...
        tmpav_pre(k)=tmpav(k)             !! used for water temperature module------------------------------------
