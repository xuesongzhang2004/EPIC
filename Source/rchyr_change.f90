...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
 ...
 !! added NO3 Concentration to output.rch (for daily only) gsm 10/26/2011
              !----assign yearly output values for carbon module,added by Du--------------!
        pdvar(47) = rchyro(63,j)              !!RPOC in (kg C)
        pdvar(48) = rchyro(64,j)              !!RPOC out (kg C)
        pdvar(49) = rchyro(65,j)              !!LPOC in (kg C)
        pdvar(50) = rchyro(66,j)              !!LPOC in (kg C)
        pdvar(51) = rchyro(67,j)              !!RDOC in (kg C)
        pdvar(52) = rchyro(68,j)              !!RDOC out (kg C)
        pdvar(53) = rchyro(69,j)              !!LDOC in (kg C)
        pdvar(54) = rchyro(70,j)              !!LDOC in (kg C)
        pdvar(55) = rchyro(71,j)              !!DIC in (kg C))
        pdvar(56) = rchyro(72,j)              !!DIC out (kg C))
        pdvar(57) = rchyro(64,j)+rchyro(66,j) !!Total POC out (kg C)
