...
!	        end if
            if (ii == 1) then
                rtwtr = c1 * wtrin + c2 * flwin(jrch)/real(nn) + c3 * flwout(jrch)/real(nn) !+ c4 * latflow
            else
                rtwtr = c1 * wtrin + c2 * flwin(jrch) + c3 * flwout(jrch) !+ c4 * latflow
            end if
