...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, j1, ii, isp, nn, ly
      real:: ul_excess,qlyr,pormm,rtof, qvol, xx
	  ...
	  do while (qlyr>0.and.ii>1)
	   ...
	   if (sol_st(ii,j) > sol_ul(ii,j)) then
	     ...
	     sol_no3(ii,j) = sol_no3(ii,j) - xx * (sptno3concs(isp)   &
	     				+ sptno2concs(isp))  
	     sol_nh4(ii,j) = sol_nh4(ii,j) - xx * sptnh4concs(isp) 
	     ...
	     sol_no3(ii-1,j) = sol_no3(ii-1,j) + xx * (sptno3concs(isp)  &
	     				+ sptno2concs(isp))  
	     sol_nh4(ii-1,j) = sol_nh4(ii-1,j) + xx * sptnh4concs(isp) 
