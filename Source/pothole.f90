...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: tileo, spillo, potflwosp, potno3o,potsolpo, potorgno, potorgpo, potmpso, potmpao, potvol_ini, potsa_ini
      real :: qin, rto, potvol_m3, xx, pot_depth, drcla, drsil, drtot, potvol_tile, potvol_sep, solp_tileo
...
!       compute evaporation from water surface
        ...
        if (potvol_tile > 1.e-6) then
          ...
          no3loss = pot_no3(j) *  tileo / potvol_tile   !tileo       |m^3 H2O       |amount of water released to the main channel
                                                        !potvol_tile = pot_vol(j)
          ...
          no3_loss_pothole(j) = no3_loss_pothole(j) + no3loss !! Zhang added
...
!     !!! output.pot and output.wtr turned on by same code named IWTR in file.cio
      if (iwtr == 1) then
        write (output_pot_num,2000) subnum(j), hruno(j), i, iyr, potvol_ini,       &
